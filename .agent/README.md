...
> exact path they write to. If your build uses a different location, move these two files
> there (their _content_ is correct regardless of path) and delete this directory. Set the
> rule's activation mode to **Model Decision** in the Rules panel — the mode is configured in
> the UI, not the file.
