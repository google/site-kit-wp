...
const siteKitLogoBase64 = fs
	.readFileSync(
		path.resolve(
			__dirname,
			'../../../assets/images/email-development/2025-12-01-site-kit-logo.png'
		)
	)
	.toString( 'base64' );

// Playwright logo SVG sourced from playwright-core package.
const playwrightLogoSvg = `<svg width="64" height="64" viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo-pw {
    ...
    align-items: center;
    justify-content: center;
    animation: spin-in 1.8s cubic-bezier(0.16, 1, 0.3, 1) 0.3s both;
  }
  ...
  .pill {
    ...
    padding: 4px 12px;
    border-radius: 999px;
    border: 1px solid rgba(46, 173, 51, 0.35);
    background: rgba(46, 173, 51, 0.08);
    color: rgba(46, 173, 51, 0.85);
    font-size: 10px;
    font-weight: 600;
    letter-spacing: 0.1em;
    text-transform: uppercase;
    animation: rise 0.4s cubic-bezier(0.16, 1, 0.3, 1) 0.1s both;
  }
  ...
  .spinner {
    ...
    height: 10px;
    border-radius: 50%;
    border: 1.5px solid rgba(46, 173, 51, 0.25);
    border-top-color: rgba(46, 173, 51, 0.85);
    animation: spin 0.8s linear 0.5s infinite both;
  }
...
<body>
  <div class="card">
    <div class="logos">
      <div class="logo-sk">
        <img src="data:image/png;base64,${ siteKitLogoBase64 }" alt="Site Kit by Google">
      </div>
      <div class="divider"></div>
      <div class="logo-pw">${ playwrightLogoSvg }</div>
    </div>
    <div class="test-title">${ safeTitle }</div>
    <div class="pill"><span class="spinner"></span> Loading...</div>
  </div>
</body>
</html>`;
}
