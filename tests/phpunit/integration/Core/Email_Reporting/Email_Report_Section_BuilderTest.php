...
class Email_Report_Section_BuilderTest extends TestCase {
	...
	public function test_build_sections__restores_locale_after_success() {
		$context = new Context( GOOGLESITEKIT_PLUGIN_MAIN_FILE );
		$builder = new Email_Report_Section_Builder( $context );
		...
		$sections = $builder->build_sections( 'analytics-4', $this->get_ga4_payloads(), $recipient_locale );

		$this->assertIsArray( $sections, 'Sections should be returned as an array on the success path.' );
		$this->assertEquals( $original_locale, get_locale(), 'Locale should be restored to its original value after a successful build.' );
	}

	public function test_build_sections__restores_locale_on_wp_error() {
		$context = new Context( GOOGLESITEKIT_PLUGIN_MAIN_FILE );
		$builder = new Email_Report_Section_Builder( $context );
		...
		$sections = $builder->build_sections( 'search-console', $payloads, $recipient_locale );

		$this->assertWPError( $sections, 'Search Console errors should be propagated as WP_Error.' );
		$this->assertEquals( $original_locale, get_locale(), 'Locale should be restored after build_sections() returns a module error.' );
	}

	public function test_build_sections__restores_locale_on_exception() {
		$context = new Context( GOOGLESITEKIT_PLUGIN_MAIN_FILE );
		$builder = $this->getMockBuilder( Email_Report_Section_Builder::class )
			->setConstructorArgs( array( $context ) )
			->onlyMethods( array( 'extract_sections_from_payloads' ) )
			->getMock();

		$builder->method( 'extract_sections_from_payloads' )
		...
		try {
			$builder->build_sections( 'analytics-4', array(), $recipient_locale );
			$this->fail( 'Expected exception was not thrown.' );
		} catch ( \Exception $exception ) {
			$this->assertSame( 'Test exception.', $exception->getMessage(), 'Original exception should be re-thrown unchanged.' );
		}

		$this->assertEquals( $original_locale, get_locale(), 'Locale should be restored after build_sections() throws.' );
	}

	public function test_build_sections__restores_locale_stack_across_multiple_modules() {
		$context = new Context( GOOGLESITEKIT_PLUGIN_MAIN_FILE );
		$builder = new Email_Report_Section_Builder( $context );
		...
		$builder->build_sections( 'analytics-4', $this->get_ga4_payloads(), $recipient_locale );
		$builder->build_sections( 'search-console', $search_console_payloads, $recipient_locale );

		$this->assertEquals( $original_locale, get_locale(), 'Locale should be restored after building sections for multiple modules in sequence.' );
	}

	/**
	 * Returns a locale different from the one given, for exercising the recipient-locale switch.
	 *
	 * @param string $locale Locale to differ from.
	 * @return string A different locale.
	 */
	private function get_other_locale( $locale ) {
		return 'en_US' === $locale ? 'de_DE' : 'en_US';
	}

	/**
	 * Builds a minimal valid GA4 payload for use across success-path tests.
	 *
	 * @return array GA4 report payloads.
	 */
	private function get_ga4_payloads() {
		return array(
			array(
				'total_visitors' => array(
					array(
