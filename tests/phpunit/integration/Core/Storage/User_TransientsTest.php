...
class User_TransientsTest extends TestCase {
	...
	protected function with_predefined_environment( $callback, $ext_object_cache ) {
		global $_wp_using_ext_object_cache;

		$old_wp_using_ext_object_cache = $_wp_using_ext_object_cache;
		$_wp_using_ext_object_cache    = 'with_ext_cache' === $ext_object_cache;
	...
	protected function using_external_cache( $callback ) {
		$this->with_predefined_environment( $callback, 'with_ext_cache' );
	}

	protected function using_user_options( $callback ) {
		$this->with_predefined_environment(
			function( $user_transients, $user_id ) use ( $callback ) {
				$user_options = new User_Options( $this->context, $user_id );
				call_user_func( $callback, $user_transients, $user_options, $user_id );
			},
			'without_ext_cache'
		);
	}
	...
	public function test_get_using_user_options() {
		$this->using_user_options(
			function( $user_transients, $user_options ) {
