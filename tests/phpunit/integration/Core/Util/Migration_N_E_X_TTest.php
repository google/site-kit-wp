...
class Migration_N_E_X_TTest extends TestCase {
	...
	public function set_up() {
		...
		// Drop the option and any filters the bootstrap plugin instance
		// added, so each test reads and writes the raw option value.
		delete_option( Connected_Proxy_URL::OPTION );
		remove_all_filters( 'option_' . Connected_Proxy_URL::OPTION );
	...
	public function test_migrate__encodes_a_legacy_plain_text_value() {
		...
		$this->assertEquals(
			...
			get_option( Connected_Proxy_URL::OPTION ),
	...
	public function test_migrate__keeps_an_encoded_value() {
		$migration = $this->get_new_migration_instance();

		$this->connected_proxy_url->set( 'https://example.com' );
		$encoded = get_option( Connected_Proxy_URL::OPTION );
