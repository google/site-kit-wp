...
class Migration_1_8_1Test extends TestCase {
	...
	public function test_migrate_without_problem_cases() {
		$this->create_user( 'administrator', self::FILE_VERIFICATION );
	...
	private function create_user( $role, $verification_type = false ) {
		...
		if ( ! empty( $verification_type ) ) {
			...
			// Set access token too to simulate user being fully authenticated.
	...
	private function assertReceivedIdentifiers( $expected ) {
	...
	private function assertNoSiteKitUserOptions( $user_id ) {
		global $wpdb;
		...
		$this->assertEmpty( $site_kit_user_options, 'Failed asserting that user has no Site Kit options.' );
	}
