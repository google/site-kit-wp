...
class Connected_Proxy_URLTest extends SettingsTestCase {
	...
	public function test_get__decodes_the_stored_value() {
		$connected_proxy_url = new Connected_Proxy_URL( $this->options );
		$connected_proxy_url->register();
	...
	public function test_sanitize_callback__encodes_a_value_saved_directly() {
		$connected_proxy_url = new Connected_Proxy_URL( $this->options );
		$connected_proxy_url->register();
	...
	public function test_matches_url__stored_value_survives_search_and_replace() {
		$connected_proxy_url = new Connected_Proxy_URL( $this->options );
		$connected_proxy_url->register();
		...
		$stored    = $this->get_raw_option_value();
		$rewritten = str_replace( 'old-site.example.com', 'new-site.example.com', $stored );

		$this->assertSame( $stored, $rewritten, 'Search and replace should not find the URL inside the encoded value.' );
	...
	/**
	 * Gets the raw option value straight from the database.
	 *
	 * The `option_` filter decodes the value on read, so this helper reads
	 * the row the way a search and replace tool sees it.
	 *
	 * @return string|null Raw option value, or NULL when the row is missing.
	 */
