...
class Get_Audience_SettingsTest extends TestCase {
	...
	public function test_create_request_returns_callable() {
		...
		$this->assertIsCallable( $request, 'Get audience settings should return a callable request handler.' );
	}
	...
	public function test_create_request_returns_full_settings_for_admin() {
		...
		$this->assertArrayHasKey( 'availableAudiences', $response, 'Admin should see availableAudiences.' );
		$this->assertArrayHasKey( 'availableAudiencesLastSyncedAt', $response, 'Admin should see availableAudiencesLastSyncedAt.' );
		$this->assertArrayHasKey( 'audienceSegmentationSetupCompletedBy', $response, 'Admin should see audienceSegmentationSetupCompletedBy.' );
	}
	...
	public function test_create_request_returns_filtered_settings_for_non_admin() {
		...
		$this->assertArrayHasKey( 'availableAudiences', $response, 'Non-admin should see availableAudiences (view-only key).' );
		$this->assertArrayHasKey( 'audienceSegmentationSetupCompletedBy', $response, 'Non-admin should see audienceSegmentationSetupCompletedBy (view-only key).' );
		$this->assertArrayNotHasKey( 'availableAudiencesLastSyncedAt', $response, 'Non-admin should not see availableAudiencesLastSyncedAt.' );
	}
