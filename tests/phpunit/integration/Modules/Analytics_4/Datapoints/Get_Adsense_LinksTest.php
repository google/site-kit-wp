<?php
/**
 * Class Google\Site_Kit\Tests\Modules\Analytics_4\Datapoints\Get_Adsense_LinksTest
 *
 ...
 * @group Datapoints
 */
class Get_Adsense_LinksTest extends TestCase {

	/**
	 * Get_Adsense_Links datapoint instance.
	 *
	 * @var Get_Adsense_Links
	 */
	private $datapoint;
	 ...
	 * @var Request
	 */
	private $get_adsense_links_request;
	...
	public function set_up() {
		...
		$this->datapoint = new Get_Adsense_Links(
			array(
		...
		FakeHttp::fake_google_http_handler(
			...
			function ( Request $request ) {
				$this->get_adsense_links_request = $request;

				$response = new GoogleAnalyticsAdminV1alphaListAdSenseLinksResponse();
				$response->setAdsenseLinks( array( new GoogleAnalyticsAdminV1alphaAdSenseLink() ) );
	...
	public function test_create_request__requires_property_id_param() {
		...
		$data_request = new Data_Request( 'GET', 'modules', 'analytics-4', 'adsense-links', $data );
	...
	public function test_create_request() {
		$this->get_adsense_links_request = null;
		...
		$this->assertEquals(
			'https://analyticsadmin.googleapis.com/v1alpha/properties/12345/adSenseLinks',
			$this->get_adsense_links_request->getUri(),
			'The request should be made to the correct endpoint.'
		);
	}

	public function test_parse_response() {
		$data_request = new Data_Request( 'GET', 'modules', 'analytics-4', 'adsense-links' );

		$adsense_links_response = new GoogleAnalyticsAdminV1alphaListAdSenseLinksResponse();
		$adsense_links_response->setAdsenseLinks( array( new GoogleAnalyticsAdminV1alphaAdSenseLink() ) );

		$response = $this->datapoint->parse_response( $adsense_links_response, $data_request );
