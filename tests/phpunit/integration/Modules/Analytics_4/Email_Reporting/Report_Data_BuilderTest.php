...
class Report_Data_BuilderTest extends TestCase {
	...
	/**
	 * Builds the report that holds the site's engagement rate and session count.
	 *
	 * @return array Report holding one row per date range.
	 */
	...
	public function test_build_sections_from_module_payload__reads_the_site_goals_reports_into_one_online_store_section() {
		...
		$this->assertSame(
			...
			'build_sections_from_module_payload() should build one Site Goals section, then one section for every other report.'
		);
	}

	public function test_build_sections_from_module_payload__builds_one_site_goals_section_rather_than_one_per_report() {
		$sections = $this->builder->build_sections_from_module_payload(
			array(
				'site_goals_online_store_primary' => $this->build_online_store_report(),
				'site_goals_engagement'           => $this->build_engagement_report(),
		...
		$this->assertSame(
			...
			array_column( $sections, 'section_key' ),
			'build_sections_from_module_payload() should give site_goals_online_store_primary and site_goals_engagement no section of their own, because the Site Goals section already holds both.'
		);
	}

	public function test_build_sections_from_module_payload__builds_one_section_for_a_report_that_is_not_a_site_goals_report() {
		$sections = $this->builder->build_sections_from_module_payload(
		...
		$this->assertSame( 'total_visitors', $sections[0]['section_key'], 'build_sections_from_module_payload() should give the total visitors section the key total_visitors.' );
