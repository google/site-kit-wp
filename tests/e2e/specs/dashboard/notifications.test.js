...
describe( 'core site notifications', () => {
	...
	describe( 'when not using proxy', () => {
		...
		it( 'does not display core site notifications on the main dashboard', async () => {
			...
			const notification_titles = await page.$$( '.googlesitekit-publisher-win__title' );
			const notification_description = await page.$$( '.googlesitekit-publisher-win__desc' );

			expect(
				notification_titles.filter( ( { textContent } ) => textContent.match( /test notification title/i ) )
			).toHaveLength( 0 );
			expect(
				notification_description.filter( ( { textContent } ) => textContent.match( /test notification content/i ) )
			).toHaveLength( 0 );
		} );
	} );
} );
