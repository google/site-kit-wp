...
echo "==> Updating root package.json packages..."

# @shopify/jest-dom-mocks: 4 → 5
# API-compatible; only adds new matchers and improves types.
npm install --save-dev @shopify/jest-dom-mocks@^5.2.0
...
npm install --save-dev stylelint@^17.11.1

# stylelint-config-standard-scss: 12 → 17
# Requires stylelint v17 (updated above). ✓
npm install --save-dev stylelint-config-standard-scss@^17.0.0

# stylelint-order: 6 → 8
# Requires stylelint v16+ (project has v17 after the update above). ✓
npm install --save-dev stylelint-order@^8.1.1

# stylelint-scss: 6 → 7
# Requires stylelint v16+ (project has v17 after the update above). ✓
npm install --save-dev stylelint-scss@^7.1.1

# webpack-cli: 6 → 7
# No programmatic usage — CLI only via npm scripts. Works with webpack v5.
npm install --save-dev webpack-cli@^7.0.2
...
npm install --save-dev webpack-manifest-plugin@^6.0.1 -w assets
...
echo ""
echo "==> Updating storybook/package.json packages..."

# @storybook/addon-webpack5-compiler-babel: 3 → 4
# v4 targets Storybook v8 (project is on 8.6.12). ✓
npm install --save-dev @storybook/addon-webpack5-compiler-babel@^4.0.1 -w storybook

echo ""
echo "==> Updating tests/e2e/package.json packages..."

# cross-env: 7 → 10
# CLI tool only; no API surface consumed programmatically.
npm install cross-env@^10.1.0 -w tests/e2e
...
npm install jsdom@^29.1.1 -w tests/e2e
...
echo ""
echo "==> Updating tests/js/package.json packages..."

# cross-env: 7 → 10 (same reasoning as tests/e2e)
npm install --save-dev cross-env@^10.1.0 -w tests/js
...
echo ""
echo "==> Updating tests/playwright/package.json packages..."

# @types/node: 22 → 24 (aligns with Node 24 LTS)
npm install --save-dev @types/node@^24.0.0 -w tests/playwright
