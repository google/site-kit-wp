...
2. **Work out which stage you're in.** If `$ARGUMENTS` names one of the numbered stage
   ...
   process as a whole. Its four preparation stages are optional: where a preparation row of
   the brief's "Current inputs" table names no path, work from the PRD and the design instead
   of asking for it or producing it yourself.
