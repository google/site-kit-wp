	...
	protected function get_normalized_phone( $phone, $country ) {
		...
		if ( class_exists( 'WC_Countries' ) && ! empty( $country ) ) {
			...
			if ( ! empty( $calling_code ) ) {
				...
				// Check if the phone number starts with 00 + country code (international format).
				$starts_with_00_and_country_code = strpos( $phone_digits, '00' . $country_code_digits ) === 0;
				...
				if ( strpos( $phone_digits, $country_code_digits ) === 0 ) {
					...
					$phone = '+' . $phone_digits;
				} elseif ( $starts_with_00_and_country_code ) {
					// Phone starts with 00 + country code (international dialing format).
