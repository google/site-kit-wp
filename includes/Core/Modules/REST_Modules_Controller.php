...
class REST_Modules_Controller {
	...
	private function get_rest_routes() {
		...
		return array(
					...
					array(
						...
						'callback'            => function( WP_REST_Request $request ) {
							...
							if ( ! $module instanceof Module_With_Data_Available_State ) {
								return new WP_Error( 'invalid_module_slug', __( 'Module does not support setting data available state.', 'google-site-kit' ), array( 'status' => 400 ) );
							}
			...
			new REST_Route(
				'modules/(?P<slug>[a-z0-9\-]+)/data/reset-data-available',
				array(
					array(
						'methods'             => WP_REST_Server::CREATABLE,
						'callback'            => function( WP_REST_Request $request ) {
							...
							return new WP_REST_Response( $module->reset_data_available() );
						},
						'permission_callback' => $can_list_data,
					),
				),
