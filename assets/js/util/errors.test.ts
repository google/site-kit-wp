...
describe( 'Error Utilities', () => {
	...
	describe.each( [
		...
		it( 'should return FALSE for non-plain objects', () => {
			...
			expect( fn( Object.create( Date.prototype ) ) ).toBe( false );
	...
	describe( 'getReportErrorMessage', () => {
		it( 'should return a custom message for internal server error', () => {
			expect(
				getReportErrorMessage( { code: ERROR_INTERNAL_SERVER_ERROR } )
			).toContain( 'critical error' );
		} );

		it( 'should return a custom message for invalid json', () => {
			expect(
				getReportErrorMessage( { code: ERROR_INVALID_JSON } )
			).toContain( 'invalid response' );
		} );

		it( 'should return the original message otherwise', () => {
			expect(
				getReportErrorMessage( { message: 'Original', code: 'other' } )
			).toBe( 'Original' );
		} );
	} );
} );
