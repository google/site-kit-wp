 ...
 * @return {string} The URL path.
 */
export function getFullURL( siteURL: string, path: string ): string {
	try {
 ...
 */
export function normalizeURL( incomingURL: unknown ): unknown {
 ...
 */
export function shortenURL( url: string, maxChars: number ): string {
	if ( ! isURL( url ) ) {
		return url;
	}
