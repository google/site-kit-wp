...
export function isWPError( obj: unknown ): obj is ErrorObject {
	return (
		...
		( typeof ( obj as ErrorObject ).code === 'string' ||
			typeof ( obj as ErrorObject ).code === 'number' ) &&
		Object.prototype.hasOwnProperty.call( obj, 'message' ) &&
		typeof ( obj as ErrorObject ).message === 'string' &&
 ...
 */
export function isPermissionScopeError(
 ...
 */
export function isInsufficientPermissionsError(
	error: ErrorObject | null | undefined
): boolean {
	return [
		ERROR_REASON_INSUFFICIENT_PERMISSIONS,
		ERROR_REASON_FORBIDDEN,
	].includes( ( error?.data as ErrorData | undefined )?.reason || '' );
}
 ...
 */
export function isAuthError( error: ErrorObject | null | undefined ): boolean {
	return !! ( error?.data as ErrorData | undefined )?.reconnectURL;
}
 ...
 *
 * @param {Object} error The error object to check.
 * @return {string|undefined} The updated error object.
 */
export function getReportErrorMessage(
	...
	if ( error?.code === ERROR_INVALID_JSON ) {
		return __(
