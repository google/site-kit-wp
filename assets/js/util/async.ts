...
interface PrioritizedTaskResult {
	task?: AsyncTask;
	result: boolean;
}
 ...
 */
export async function racePrioritizedAsyncTasks(
	tasks: AsyncTask[]
): Promise< AsyncTask | null > {
	// Group tasks by priority
	const priorityGroups = tasks.reduce<
		Record< number, Array< Promise< PrioritizedTaskResult > > >
	>( ( grouped, task ) => {
	...
	const priorities = Object.keys( priorityGroups ).sort(
	...
	for ( const priority of priorities ) {
		...
		let group = [ ...priorityGroups[ Number( priority ) ] ];
		do {
 ...
 */
async function isPromiseResolved(
...
export function asyncRequireAll< Args extends unknown[] >(
	...requirements: Array< ( ...args: Args ) => Promise< boolean > | boolean >
 ...
 */
export function asyncRequireAny< Args extends unknown[] >(
	...requirements: Array< ( ...args: Args ) => Promise< boolean > | boolean >
): ( ...args: Args ) => Promise< boolean > {
	return async function ( ...args: Args ) {
		for ( const predicate of requirements ) {
 ...
 */
export function asyncRequire< Args extends unknown[], T >(
	want: T,
	func: ( ...args: Args ) => Promise< T > | T
): ( ...args: Args ) => Promise< boolean > {
	return async function ( ...args: Args ) {
