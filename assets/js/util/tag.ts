...
interface GetExistingTagURLsArgs {
	homeURL: string;
	ampMode?: string;
}
 ...
 */
export function extractExistingTag(
	html: string,
	tagMatchers: RegExp[]
): string | false {
	...
	if ( matchingPattern ) {
		return matchingPattern.exec( html )?.[ 1 ] || false;
	}
 ...
 */
export const getExistingTagURLs = memoize(
	async ( {
		homeURL,
		ampMode,
	}: GetExistingTagURLsArgs ): Promise< string[] > => {
