...
import { NumFmtOptions } from '@/js/util/i18n';

function setupGoogleSiteKit( langCode: string ) {
...
describe( 'numberFormat', () => {
	...
	const siteKitLocales: Array< [ string, number, string ] > = [
	...
	describe( 'graceful degradation for problematic options in some browsers', () => {
		...
		function createThrowIfOptionMatch(
			key: keyof NumFmtOptions,
			value: unknown
		) {
			return ( locales?: string, options: NumFmtOptions = {} ) => {
				if (
					options[ key ] &&
