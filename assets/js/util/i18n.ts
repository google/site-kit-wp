...
/**
 * Loose formatting options shared across the number/duration formatting
 * helpers below. A handful of commonly used fields are explicitly typed;
 * an index signature allows any other `Intl.NumberFormatOptions` (or Site
 * Kit's own shorthand values, e.g. `style: 'metric'`) to pass through.
 */
export type NumFmtOptions = {
	locale?: string;
	style?: string;
	unitDisplay?: string;
...
function durationISOFormat( durationInSeconds: number ): string {
	const parsed = parseDuration( durationInSeconds );

	const seconds = ( '0' + parsed.seconds ).slice( -2 );
	const minutes = ( '0' + parsed.minutes ).slice( -2 );
	const hours = ( '0' + parsed.hours ).slice( -2 );
...
function parseDuration( durationInSeconds: number ): {
	...
	let parsedDurationInSeconds = parseInt( String( durationInSeconds ), 10 );

	if ( Number.isNaN( parsedDurationInSeconds ) ) {
		parsedDurationInSeconds = 0;
	}

	const hours = Math.floor( parsedDurationInSeconds / 60 / 60 );
	const minutes = Math.floor( ( parsedDurationInSeconds / 60 ) % 60 );
	const seconds = Math.floor( parsedDurationInSeconds % 60 );
...
export function expandNumFmtOptions(
	...
	if ( '%' === options ) {
		...
		formatOptions = { ...( options as NumFmtOptions ) };
	}
...
export function numFmt(
	...
	if ( 'duration' === style ) {
		return durationFormat( number as number, formatOptions );
	}

	if ( 'durationISO' === style ) {
		return durationISOFormat( number as number );
	}

	return numberFormat( number as number, formatOptions );
}
...
export function numberFormat(
	...
	const unstableFormatOptionValues: Record< string, string > = {
	...
	const reducedFormatOptions: Record< string, unknown > = {};
	...
	try {
		return new Intl.NumberFormat(
			locale,
			reducedFormatOptions as Intl.NumberFormatOptions
		).format( number );
	} catch {
...
export function listFormat(
	...
	// `Intl.ListFormat` is not in the configured TS `lib`, so it's accessed
	// via `any` here rather than widening the project-wide `lib` setting.
 ...
 *                  E.g. `en-US` or `de-DE`
 */
export function getLocale( _global: typeof globalThis = global ): string {
...
/**
 * Options for the display name formatters below.
 */
 ...
 */
export function languageCodeFormat(
	languageCode: string,
 ...
 */
export function regionCodeFormat(
	regionCode: string,
	options: DisplayNamesOptions = {}
): string {
	const { locale = getLocale() } = options;
