...
interface DifferenceArrowOptions {
	invertColor?: boolean;
}

interface ChartReportRow {
	metrics?: Array< { values?: unknown[] } >;
}

interface ChartReportGroup {
	data?: {
		rows?: ChartReportRow[];
		totals?: Array< { values?: unknown[] } >;
	};
}
 ...
 */
export function getChartDifferenceArrow(
	difference: number,
	options: DifferenceArrowOptions = {}
): string {
 ...
 */
export function isSingleSlice(
	report: ChartReportGroup[] | undefined
): boolean | undefined {
	if ( report === undefined ) {
		return undefined;
	}
