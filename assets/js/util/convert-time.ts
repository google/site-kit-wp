 ...
 */
export function convertSecondsToArray( seconds: unknown ): number[] {
	const parsedSeconds = parseFloat( String( seconds ) );

	if ( isNaN( parsedSeconds ) || 0 === parsedSeconds ) {
		return [ 0, 0, 0, 0 ];
	}

	return [
		Math.floor( parsedSeconds / 60 / 60 ),
		Math.floor( ( parsedSeconds / 60 ) % 60 ),
		Math.floor( parsedSeconds % 60 ),
		Math.floor( parsedSeconds * 1000 ) - Math.floor( parsedSeconds ) * 1000,
	];
}
 ...
 */
export function convertDateStringToUNIXTimestamp(
	dateStringValue: unknown
): number {
	const unixTimestamp =
		dateStringValue && ! Number.isInteger( dateStringValue )
			? // Valid use of `new Date()` with an argument, because this should only
			  ...
			  new Date( dateStringValue as string | number ).getTime()
			: ( dateStringValue as number );
