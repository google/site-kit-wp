...
} from './insufficient-permissions-error-description';
...
describe( 'getInsufficientPermissionsErrorDescription', () => {
	...
	describe( 'invalid module property', () => {
		// Deliberately malformed module shapes (wrong property types), so
		// these are typed loosely rather than as `Module`.
	...
	describe( 'not supported module', () => {
		...
		const cases: Array< [ string, Module, string ] > = [
			[
