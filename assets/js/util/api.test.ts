...
describe( 'trackAPIError', () => {
	let dataLayerPushSpy: jest.SpyInstance;

	beforeEach( () => {
		enableTracking();
		( global as Record< string, unknown > )[ DATA_LAYER ] = [];
		dataLayerPushSpy = jest.spyOn(
			( global as unknown as Record< string, unknown[] > )[ DATA_LAYER ],
			'push'
		);
	} );
