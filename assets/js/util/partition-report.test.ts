 ...
 */
import { PartitionReportArgs, partitionReport } from './partition-report';

describe( 'partitionReport', () => {
	it( 'it requires an array', () => {
		expect( () =>
			partitionReport( {} as unknown as unknown[], {
				dateRangeLength: 1,
			} )
		).toThrow( 'report must be an array' );
		expect( () =>
	...
	it( 'requires dateRangeLength to be a positive integer', () => {
		expect( () =>
			partitionReport( [], {} as unknown as PartitionReportArgs )
		).toThrow( 'dateRangeLength must be a positive integer' );
	...
	describe( 'partitions the given report into a currentRange and compareRange based on the dateRangeLength', () => {
		...
		it( 'partitions first items into compareRange and second items into currentRange', () => {
			...
			const report = [ ...firstThree, ...secondThree ];
		...
		it( 'ensures weekdays still line up as expected when last day is missing (if week(s) are requested)', () => {
			...
			const report = [ ...previous7Days, ...last7DaysWithoutYesterday ];
			...
			// arrays align.
			function mapToWeekday( dataset: unknown ) {
				return ( dataset as { weekday: string } ).weekday;
			}
