...
interface WhenScopesGrantedOwnProps< P > {
 ...
 * @param {Array}       options.scopes              Array of scopes to check.
 ...
 */
export default function whenScopesGranted< P >( {
	scopes = [],
	FallbackComponent,
}: {
	scopes?: string[];
	...
	return ( WrappedComponent: ComponentType< P > ) => {
		const WhenScopesGranted: FC< P & WhenScopesGrantedOwnProps< P > > = (
			props
		) => {
