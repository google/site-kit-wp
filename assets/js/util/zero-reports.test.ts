 ...
 */
import mockedReportResponse from '@/js/modules/analytics-4/utils/__fixtures__/mocked-report-multiple-distinct-date-ranges.json';
import {
	Analytics4Report,
...
describe( 'zeroReports', () => {
	describe( 'replaceValuesOrRemoveRowForDateRangeInAnalyticsReport', () => {
		...
		it( 'should zero values for date_range_0 when emptyRowBehavior is "zero"', () => {
			...
			const report: Analytics4Report = JSON.parse(
				JSON.stringify( mockedReportResponse )
			);
			const result =
				replaceValuesOrRemoveRowForDateRangeInAnalyticsReport(
					report,
			...
			dateRangeOne.forEach( ( row, index ) => {
				const originalRow = report.rows!.filter(
					( r: Analytics4ReportCell ) =>
			...
			const originalTotalDateRangeOne = report.totals!.find(
				( total: Analytics4ReportCell ) =>
					matchesDateRange( total.dimensionValues!, 'date_range_1' )
			);
		...
		it( 'should zero values for date_range_1 when emptyRowBehavior is "zero"', () => {
			...
			const originalTotalDateRangeZero = report.totals!.find(
				( total: Analytics4ReportCell ) =>
					matchesDateRange( total.dimensionValues!, 'date_range_0' )
			);
