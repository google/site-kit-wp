...
interface WhenActiveOwnProps< P > {
 ...
 */
export default function whenActive< P >( {
	moduleName,
	FallbackComponent,
	IncompleteComponent,
}: {
	...
	IncompleteComponent?: ComponentType< P >;
} ) {
	return ( WrappedComponent: ComponentType< P > ) => {
		const WhenActiveComponent: FC< P & WhenActiveOwnProps< P > > = (
			props
		) => {
