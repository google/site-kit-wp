...
export function replaceValuesOrRemoveRowForDateRangeInAnalyticsReport(
	...
	function matchesDateRange( cell: any ) {
		return cell.dimensionValues?.some(
			( value: any ) => value.value === dateRangeKey
		);
	}
 ...
 */
export function replaceValuesInAdSenseReportWithZeroData( report: any ): any {
	function zeroValue( cell: any ) {
		return { ...cell, value: 0 };
	}
	...
	clonedReport = {
		...
		rows: rows.map( ( row: any ) => ( {
			...row,
			cells: row.cells.map( ( cell: any, index: number ) => {
				if ( index !== 0 ) {
 ...
 * @param {string} options.dimensions Search Console dimensions.
