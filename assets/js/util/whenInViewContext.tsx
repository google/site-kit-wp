 ...
 * @param {Array}   options.includeList    Array of view contexts that the component should render when present.
 * @param {Array}   options.excludeList    Array of view contexts that the component should render when not present.
 * @return {Function} Enhancing function.
 */
export default function whenInViewContext< P extends object >( {
	allViewOnly,
	allNonViewOnly,
	includeList,
	excludeList,
	...
	allViewOnly?: boolean;
	allNonViewOnly?: boolean;
	includeList?: string[];
	excludeList?: string[];
} ) {
	return ( WrappedComponent: ComponentType< P > ) => {
		const WhenInViewContext: FC< P > = ( props ) => {
