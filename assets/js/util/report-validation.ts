...
type VerifyObjectFunction = ( data: unknown ) => boolean;
type VerifyStringFunction = ( data: string ) => boolean;
...
export function isValidStringsOrObjects(
	data:
		...
		| Record< string, unknown >[],
	verifyObjectFunction: VerifyObjectFunction,
	verifyStringFunction: VerifyStringFunction = () => true
): boolean {
...
export function isValidDateRange( {
	...
	return Boolean( validStartDate && validEndDate );
}
 ...
 */
export function isValidOrders( orders: unknown ): boolean {
	function isValidOrder( order: Record< string, unknown > ): boolean {
		const isValidFieldName =
			Object.prototype.hasOwnProperty.call( order, 'fieldName' ) &&
			!! order.fieldName;
		const isValidSortOrder =
			Object.prototype.hasOwnProperty.call( order, 'sortOrder' ) &&
			/(ASCENDING|DESCENDING)/i.test( String( order.sortOrder ) );
 ...
 */
export function isValidStringularItems( items: string | string[] ): boolean {
