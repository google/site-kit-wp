...
interface TrackingDataLayerTarget {
	console?: Console;
}
...
export default function createTracking(
	...
	dataLayerTarget: TrackingDataLayerTarget = global,
	_global: TrackingDataLayerTarget = global
) {
	...
	const onceTrackedEventsMap: Record<
		string,
		( ...args: Parameters< typeof trackEvent > ) => void
	> = {};

	function trackEventOnce( ...params: Parameters< typeof trackEvent > ) {
		const key = JSON.stringify( params );
		if ( ! onceTrackedEventsMap[ key ] ) {
			onceTrackedEventsMap[ key ] = once(
				trackEvent
			) as typeof onceTrackedEventsMap[ string ];
		}
