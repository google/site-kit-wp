	...
	describe( 'actions', () => {
		describe( 'setModuleSettingsPanelState', () => {
			it( 'sets module settings panel state', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'view' );
				expect( store.getState().settingsPanel ).toEqual( { ...expectedInitialState, currentModule: slug, isEditing: false } );

				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'edit' );
				expect( store.getState().settingsPanel ).toEqual( { ...expectedInitialState, currentModule: slug, isEditing: true } );

				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'closed' );
			...
			it( 'should not change the panel state when called with an invalid value', () => {
				...
				expect( () => {
					registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'invalid' );
	...
	describe( 'selectors', () => {
		describe( 'getModuleSettingsPanelState', () => {
			it( 'returns "closed" by default', () => {
				expect( registry.select( CORE_MODULES ).getModuleSettingsPanelState( slug ) ).toBe( 'closed' );
			} );

			it( 'returns the settings panel state for a module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'view' );
				registry.select( CORE_MODULES ).getModuleSettingsPanelState( slug );
				expect( registry.select( CORE_MODULES ).getModuleSettingsPanelState( slug ) ).toBe( 'view' );

				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'closed' );
				expect( registry.select( CORE_MODULES ).getModuleSettingsPanelState( slug ) ).toBe( 'closed' );

				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'edit' );
				expect( registry.select( CORE_MODULES ).getModuleSettingsPanelState( slug ) ).toBe( 'edit' );
			} );
		} );
		...
		describe( 'isModuleSettingsPanelOpen', () => {
			it( 'returns true when module settings panel is in view state for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'view' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelOpen( slug ) ).toBe( true );
			} );
			...
			it( 'returns false when module settings panel is closed for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'closed' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelOpen( slug ) ).toBe( false );
			} );
		} );
		...
		describe( 'isModuleSettingsPanelClosed', () => {
			it( 'returns true when module settings panel is closed for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'closed' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelClosed( slug ) ).toBe( true );
			} );

			it( 'returns false when module settings panel is in view state for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'view' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelClosed( slug ) ).toBe( false );
			} );
		...
		describe( 'isModuleSettingsPanelEdit', () => {
			it( 'returns true when module settings panel is in edit state for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'edit' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelEdit( slug ) ).toBe( true );
			} );

			it( 'returns false when module settings panel is in view state for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'view' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelEdit( slug ) ).toBe( false );
			} );
		...
		describe( 'isModuleSettingsPanelLocked', () => {
			it( 'returns true when module settings panel is locked for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( 'another-slug', 'edit' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelLocked( slug ) ).toBe( true );
			} );

			it( 'returns false when module settings panel is not locked for a given module', () => {
				registry.dispatch( CORE_MODULES ).setModuleSettingsPanelState( slug, 'edit' );
				expect( registry.select( CORE_MODULES ).isModuleSettingsPanelLocked( slug ) ).toBe( false );
			} );
