...
export function registerDefaults( widgetsAPI ) {
	...
	widgetsAPI.registerWidgetArea(
		AREA_MAIN_DASHBOARD_SITE_GOALS_PRIMARY,
		{
			...
			pdfTitle: __( 'Site goals', 'google-site-kit' ),
			subtitle: __(
	...
	// `keyMetricsPDFSection` renders nothing on the dashboard. The PDF export
	// reads it to build the Key Metrics section from the metrics the user
	// selected.
	widgetsAPI.registerWidget(
		'keyMetricsPDFSection',
		{
			...
			pdf: {
				...
				// `getKeyMetrics()` returns `undefined` while the key metrics
				// load, so `isActive` reads an empty list. The export leaves
				// the Key Metrics section out. `PDFSectionsSelectionPanel`
				// selects the section once the key metrics arrive.
				isActive: ( select ) =>
