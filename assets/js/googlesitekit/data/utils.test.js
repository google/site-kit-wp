	...
	describe( 'createStrictSelect', () => {
		...
		describe( 'strictSelect', () => {
			...
			it( 'throws an error if a strict selector returns undefined', () => {
				expect(
					registry.select( STORE_NAME ).getFoo()
				).toBeUndefined();
			...
			it( 'only generates strict selectors once for each store', () => {
				expect( strictSelect( STORE_NAME ) ).toStrictEqual(
					strictSelect( STORE_NAME )
				);
				expect(
					createStrictSelect( registry.select )( STORE_NAME )
				).toStrictEqual( strictSelect( STORE_NAME ) );
			} );
		} );
	} );
