	...
	describe( 'actions', () => {
		describe( 'setDateRange', () => {
			...
			it( 'should set the date range', () => {
				...
				expect( registry.select( STORE_NAME ).getDateRange() ).toEqual(
					someDateRange
				);
			} );
		} );
		...
		describe( 'setReferenceDate', () => {
			...
			it( 'should set the reference date', () => {
				...
				expect(
					registry.stores[ STORE_NAME ].store.getState().referenceDate
	...
	describe( 'selectors', () => {
		...
		describe( 'getDateRangeDates', () => {
			...
			const createDateRangeTest = (
				...
				expect(
					registry.select( STORE_NAME ).getDateRangeDates( {
						...options,
						...additionalOptions,
					} )
				).toEqual( expected );
		...
		describe( 'getDateRangeNumberOfDays', () => {
			const createNumberOfDaysTest = (
				...
				registry.dispatch( STORE_NAME ).setDateRange( dateRange );
				expect(
					registry.select( STORE_NAME ).getDateRangeNumberOfDays()
		...
		describe( 'getReferenceDate', () => {
			it( 'should return the reference date once set', () => {
				...
				registry
					...
					.setReferenceDate( someReferenceDate );
				expect(
					registry.select( STORE_NAME ).getReferenceDate()
				).toEqual( someReferenceDate );
			} );
