	...
	describe( 'actions', () => {
		describe( 'dismissTour', () => {
			...
			it( 'requires a slug parameter', () => {
				expect( () => registry.dispatch( CORE_USER ).dismissTour() )
		...
		describe( 'receiveAllFeatureTours', () => {
			it( 'requires tours to be an array', () => {
				expect( () => registry.dispatch( CORE_USER ).receiveAllFeatureTours() )
		...
		describe( 'receiveFeatureToursForView', () => {
			it( 'requires viewTours to be an array', () => {
				expect( () => registry.dispatch( CORE_USER ).receiveFeatureToursForView() )
			...
			it( 'receives the given viewTours into the state for the viewContext', () => {
				const tours = [ testTourA, testTourB ];
				registry.dispatch( CORE_USER ).receiveFeatureToursForView( tours, { viewContext: 'foo' } );
		...
		describe( 'receiveLastDismissedAt', () => {
			it( 'requires a timestamp to be provided', () => {
				expect( () => registry.dispatch( CORE_USER ).receiveLastDismissedAt() )
					.toThrow( 'A timestamp is required.' );
			} );

			it( 'sets the lastDismissedAt timestamp in the store', () => {
				const timestamp = Date.now();
				registry.dispatch( CORE_USER ).receiveLastDismissedAt( timestamp );
		...
		describe( 'setLastDismissedAt', () => {
			...
			it( 'sets the lastDismissedAt timestamp in the store', async () => {
				const timestamp = Date.now();
				await registry.dispatch( CORE_USER ).setLastDismissedAt( timestamp );
				expect( store.getState().lastDismissedAt ).toEqual( timestamp );
			} );
	...
	describe( 'selectors', () => {
		...
		describe( 'getDismissedFeatureTourSlugs', () => {
			it( 'returns the initial state before the resolver runs', () => {
				...
				expect( registry.select( CORE_USER ).getDismissedFeatureTourSlugs() ).toBe( initialState.dismissedTourSlugs );
			} );
			...
			it( 'receives dismissed tours from the fetch dispatched by the resolver', async () => {
				...
				await untilResolved( registry, CORE_USER ).getDismissedFeatureTourSlugs();

				expect( registry.select( CORE_USER ).getDismissedFeatureTourSlugs() ).toEqual( [ 'feature-x' ] );
			...
			it( 'returns the list of dismissed tours', () => {
				...
				expect( registry.select( CORE_USER ).getDismissedFeatureTourSlugs() ).toEqual( [] );

				registry.dispatch( CORE_USER ).receiveGetDismissedTours( [ 'tour-a', 'feature-x' ] );
		...
		describe( 'getFeatureToursForView', () => {
			...
			it( 'returns `undefined` while tour readiness is being resolved', () => {
				expect(
					registry.select( CORE_USER ).getFeatureToursForView( 'test-view-context' ),
				).toBeUndefined();
			} );

			it( 'returns an array of tours that qualify for the given view context', async () => {
				registry.dispatch( CORE_USER ).receiveAllFeatureTours( [ testTourA, testTourB ] );

				expect(
					await registry.__experimentalResolveSelect( CORE_USER ).getFeatureToursForView( 'common-context' ),
				).toEqual( [ testTourA, testTourB ] );

				expect(
					await registry.__experimentalResolveSelect( CORE_USER ).getFeatureToursForView( 'b-only-context' ),
				).toEqual( [ testTourB ] );
			} );
			...
			it( 'returns an array of tours that have a version greater than the user’s initial Site Kit version', async () => {
				...
				registry.dispatch( CORE_USER ).receiveInitialSiteKitVersion( initialVersion );
				...
				const viewTours = await registry.__experimentalResolveSelect( CORE_USER ).getFeatureToursForView( 'common-context' );
			...
			it( 'returns an array of tours that have not been dismissed by the user yet', async () => {
				registry.dispatch( CORE_USER ).receiveAllFeatureTours( [ testTourA, testTourB ] );
				registry.dispatch( CORE_USER ).receiveGetDismissedTours( [ testTourB.slug ] );
		...
		describe( 'isTourDismissed', () => {
			it( 'returns `true` if the given slug is in the current list of dismissed tours', () => {
				...
				expect( registry.select( CORE_USER ).isTourDismissed( 'feature-x' ) ).toBe( false );

				registry.dispatch( CORE_USER ).receiveGetDismissedTours( [ 'feature-x', 'tour-y' ] );

				expect( registry.select( CORE_USER ).isTourDismissed( 'feature-x' ) ).toBe( true );
			} );
			...
			it( 'returns `undefined` if dismissed tours are not resolved yet', () => {
				...
				fetchMock.getOnce( fetchGetDismissedToursRegExp, { body: [ 'feature-x' ] } );
				expect( registry.select( CORE_USER ).isTourDismissed( 'feature-x' ) ).toBeUndefined();
			} );
		} );
		...
		describe( 'getLastDismissedAt', () => {
			...
			it( 'returns initial state (undefined) if there is no lastDismissedAt timestamp', () => {
				const lastDismissedAt = registry.select( CORE_USER ).getLastDismissedAt();
			...
			it( 'returns the lastDismissedAt timestamp if there is one', () => {
				const timestamp = Date.now();
				registry.dispatch( CORE_USER ).receiveLastDismissedAt( timestamp );
				expect( registry.select( CORE_USER ).getLastDismissedAt() ).toEqual( timestamp );
			} );
			...
			it( 'uses a resolver to set lastDismissedAt in the store if there is a value in the cache', async () => {
				...
				expect( registry.select( CORE_USER ).getLastDismissedAt() ).toBe( timestamp );
			} );
			...
			it( 'returns false for an expired lastDismissedAt value in the cache', async () => {
				...
				expect( registry.select( CORE_USER ).getLastDismissedAt() ).toBe( null );
			} );
		} );

		describe( 'areFeatureToursOnCooldown', () => {
			it( 'returns undefined if there is no lastDismissedAt timestamp', () => {
				expect( registry.select( CORE_USER ).areFeatureToursOnCooldown() ).toBeUndefined();
			} );
			...
			it( 'returns true if the lastDismissedAt timestamp is within the feature tour cooldown period', () => {
				...
				registry.dispatch( CORE_USER ).receiveLastDismissedAt( justInsideCoolDownPeriod );

				expect( registry.select( CORE_USER ).areFeatureToursOnCooldown() ).toEqual( true );
			} );
			...
			it( 'returns false if the feature tour cooldown period has expired', () => {
				...
				registry.dispatch( CORE_USER ).receiveLastDismissedAt( startOfCoolDownPeriod );

				expect( registry.select( CORE_USER ).areFeatureToursOnCooldown() ).toEqual( false );
			} );
