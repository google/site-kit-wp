	...
	describe( 'actions', () => {
		describe( 'setTrackingEnabled', () => {
			it.each( [
				...
				await registry.dispatch( CORE_USER ).setTrackingEnabled( enabled );
				...
				expect( registry.select( CORE_USER ).isTrackingEnabled() ).toBe( enabled );
			...
			it( 'dispatches an error if the request fails ', async () => {
				...
				await registry.dispatch( CORE_USER ).setTrackingEnabled( ...args );
				expect( registry.select( CORE_USER ).getErrorForAction( 'setTrackingEnabled', args ) ).toMatchObject( response );
	...
	describe( 'selectors', () => {
		describe( 'isTrackingEnabled', () => {
			it( 'should use a resolver to make a network request', async () => {
				...
				const { isTrackingEnabled, hasFinishedResolution } = registry.select( CORE_USER );
			...
			it( 'should not make a network request if data is already in state', async () => {
				...
				const tracking = registry.select( CORE_USER ).isTrackingEnabled();
				await subscribeUntil( registry, () => registry.select( CORE_USER ).hasFinishedResolution( 'isTrackingEnabled' ) );
			...
			it( 'should dispatch an error if the request fails', async () => {
				...
				const enabled = registry.select( CORE_USER ).isTrackingEnabled();
