	...
	describe( 'actions', () => {
		...
		describe( 'receiveGetDeveloperPluginState', () => {
			it( 'requires the response param', () => {
				expect( () => {
					registry.dispatch( CORE_SITE ).receiveGetDeveloperPluginState();
				} ).toThrow( 'response is required.' );
			} );

			it( 'receives and sets developer plugin state', () => {
				registry.dispatch( CORE_SITE ).receiveGetDeveloperPluginState( responseDeveloperPluginState );

				const { developerPluginState } = registry.stores[ CORE_SITE ].store.getState();
	...
	describe( 'selectors', () => {
		describe( 'getDeveloperPluginState', () => {
			it( 'uses a resolver to make a network request', async () => {
				...
				const initialDeveloperPluginState = registry.select( CORE_SITE ).getDeveloperPluginState();
				...
				await untilResolved( registry, CORE_SITE ).getDeveloperPluginState();

				const developerPluginState = registry.select( CORE_SITE ).getDeveloperPluginState();
