...
describe( 'dataAPI', () => {
	...
	describe( 'combinedGet', () => {
		...
		it( 'should call trackEvent for error in combinedGet with one error', async () => {
			...
			fetchMock.post( /^\/google-site-kit\/v1\/data/, response );
			...
			expect( eventData.event_category ).toEqual( 'api_error' );
			expect( eventData.event_label ).toEqual(
				'Internal server error (code: internal_server_error, reason: internal_server_error)'
			);
			expect( eventData.value ).toEqual( 500 );
		} );
		...
		it( 'should call trackEvent for each error in combinedGet with multiple errors', async () => {
			...
			[
				...
				eventName,
				eventData,
			] = dataLayerPushSpy.mock.calls[ 1 ][ 0 ];
			expect( event ).toEqual( 'event' );
			expect( eventName ).toEqual(
				'GET:test-type/analytics/data/test-datapoint-3'
			);
			expect( eventData.event_category ).toEqual( 'api_error' );
			expect( eventData.event_label ).toEqual(
				'Unknown error (code: unknown_error, reason: unknown_error)'
			);
