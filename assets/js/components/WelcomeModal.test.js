...
describe( 'WelcomeModal', () => {
	...
	it( 'should focus the primary button on mount for the dashboard tour variant', async () => {
		provideModules( registry, [
			{
				slug: MODULE_SLUG_ANALYTICS_4,
				active: false,
		...
		const primaryButton = getByRole( 'button', { name: 'Take a tour' } );
		expect( primaryButton ).toBeInTheDocument();
		expect( primaryButton ).toHaveFocus();
	} );

	it( 'should focus the primary button on mount for the gathering data variant', async () => {
		provideModules( registry, [
			{
				slug: MODULE_SLUG_ANALYTICS_4,
				active: false,
		...
		const primaryButton = getByRole( 'button', { name: 'Get started' } );
		expect( primaryButton ).toBeInTheDocument();
		expect( primaryButton ).toHaveFocus();
	} );
} );
