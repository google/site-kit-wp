...
/**
 * The `@react-pdf` family name for the Arabic-script font.
 *
 * `registerPDFFonts()` registers Noto Sans Arabic under it. The complex-script
 * registry (`getComplexScript`) applies it as the sole family for Arabic and
 * Persian text, which `@react-pdf` cannot shape or reorder within a font stack.
 *
 * @since n.e.x.t
 */
