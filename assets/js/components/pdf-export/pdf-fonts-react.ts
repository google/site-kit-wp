 ...
 * Arabic as the fallback for Arabic-script locales. Every binary is a local
 * webpack `asset/resource` emitted as a hashed file under `dist/fonts/`, so none
 * is inlined into the JS bundle. @react-pdf fetches a family's file only when a
 * rendered text node references it, so a Latin PDF never requests the Arabic
 * fallback and only an Arabic-script PDF fetches it. All loading happens at
 ...
 *
 * @since 1.182.0
 * @since n.e.x.t Added the Google Sans Display medium (500) weight and the Noto Sans Arabic script fallback.
 *
