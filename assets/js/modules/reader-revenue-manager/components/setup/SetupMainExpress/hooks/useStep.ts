/**
 * Reader Revenue Manager express setup `useStep` hook.
 *
...
/**
 * Returns the current express setup step slug and a setter.
 *
 ...
 *
 * Which step a flow opens on, and where it navigates to, is owned by the flow
 * itself through `useSetupFlow()`.
 *
 * @since 1.187.0
 * @since n.e.x.t Reduced to reading and writing the step, without resolving it.
 *
...
export default function useStep(): [
	...
	const [ queryArg, setQueryArg ] = useQueryArg< string >( 'step' );

	const step: string | undefined = useSelect(
		( select: Select ) =>
