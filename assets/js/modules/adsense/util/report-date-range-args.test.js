...
describe( 'AdSense report date range: generateDateRangeArgs', () => {
	it( 'should throw if a `dates` object is not provided', () => {
		expect( () => generateDateRangeArgs() ).toThrow( 'A dates object is required.' );
	} );

	it( 'should throw if `startDate` argument is not provided', () => {
		expect( () => generateDateRangeArgs( { endDate: '2021-01-27' } ) ).toThrow( 'A valid startDate is required.' );
	} );

	it( 'should throw if `endDate` argument is not provided', () => {
		expect( () => generateDateRangeArgs( { startDate: '2021-01-01' } ) ).toThrow( 'A valid endDate is required.' );
	} );
