	...
	describe( 'actions', () => {
		describe( 'receiveIsAdBlockerActive', () => {
			it( 'requires the isAdBlockerActive param to be boolean', () => {
				expect( () => {
					registry.dispatch( MODULES_ADSENSE ).receiveIsAdBlockerActive();
			...
			it( 'receives and sets isAdBlockerActive', () => {
				...
				expect( registry.select( MODULES_ADSENSE ).isAdBlockerActive() ).toBe( true );
			} );
		} );
	} );
	...
	describe( 'selectors', () => {
		describe( 'isAdBlockerActive', () => {
			it( 'uses a resolver to query detection using detectAnyAdblocker', async () => {
				...
				expect( registry.select( MODULES_ADSENSE ).isAdBlockerActive() ).toBeUndefined();
				await untilResolved( registry, MODULES_ADSENSE ).isAdBlockerActive();

				expect( registry.select( MODULES_ADSENSE ).isAdBlockerActive() ).toBe( false );
