	...
	describe( 'actions', () => {
		describe( 'submitChanges', () => {
			it( 'dispatches saveSettings', async () => {
				registry.dispatch( MODULES_ADSENSE ).setSettings( validSettings );
				fetchMock.postOnce(
				...
				expect( registry.select( MODULES_ADSENSE ).haveSettingsChanged() ).toBe( false );
			} );
			...
			it( 'handles an error if set while saving settings', async () => {
				...
				expect( registry.select( MODULES_ADSENSE ).getSettings() ).toEqual( validSettings );
				expect( registry.select( MODULES_ADSENSE ).getErrorForAction( 'submitChanges' ) ).toEqual( wpError );
		...
		describe( 'receiveOriginalAccountStatus', () => {
			it( 'requires the originalAccountStatus param', () => {
				expect( () => {
					registry.dispatch( MODULES_ADSENSE ).receiveOriginalAccountStatus();
				} ).toThrow( 'originalAccountStatus is required.' );
			} );

			it( 'receives and sets originalAccountStatus from parameter', () => {
				registry.dispatch( MODULES_ADSENSE ).receiveOriginalAccountStatus( 'something' );
				expect( registry.select( MODULES_ADSENSE ).getOriginalAccountStatus() ).toEqual( 'something' );
			} );
		} );
	} );
	...
	describe( 'selectors', () => {
		describe( 'isDoingSubmitChanges', () => {
			it( 'sets internal state while submitting changes', () => {
				expect( registry.select( MODULES_ADSENSE ).isDoingSubmitChanges() ).toBe( false );

				registry.dispatch( MODULES_ADSENSE ).submitChanges();
				expect( registry.select( MODULES_ADSENSE ).isDoingSubmitChanges() ).toBe( true );
			} );

			it( 'toggles the internal state again once submission is completed', async () => {
				const submitPromise = registry.dispatch( MODULES_ADSENSE ).submitChanges();
				expect( registry.select( MODULES_ADSENSE ).isDoingSubmitChanges() ).toBe( true );
		...
		describe( 'canSubmitChanges', () => {
			it( 'requires a valid accountID or empty string', () => {
				registry.dispatch( MODULES_ADSENSE ).setSettings( validSettings );
				expect( registry.select( MODULES_ADSENSE ).canSubmitChanges() ).toBe( true );

				registry.dispatch( MODULES_ADSENSE ).setAccountID( '0' );
				expect( () => registry.select( MODULES_ADSENSE ).__dangerousCanSubmitChanges() )
		...
		describe( 'getOriginalAccountStatus', () => {
			it( 'uses a resolver to make a network request via getSettings', async () => {
				...
				const initialOriginalAccountStatus = registry.select( MODULES_ADSENSE ).getOriginalAccountStatus();
				// Settings will be their initial value while being fetched.
				expect( initialOriginalAccountStatus ).toEqual( undefined );

				await subscribeUntil( registry, () => registry.select( MODULES_ADSENSE ).hasFinishedResolution( 'getOriginalAccountStatus' ) && registry.select( MODULES_ADSENSE ).hasFinishedResolution( 'getSettings' ) );

				const originalAccountStatus = registry.select( MODULES_ADSENSE ).getOriginalAccountStatus();
			...
			it( 'does not make a network request if original account status is already set', async () => {
				...
				registry.dispatch( MODULES_ADSENSE ).receiveOriginalAccountStatus( value );

				expect( registry.select( MODULES_ADSENSE ).getOriginalAccountStatus() ).toEqual( value );

				await subscribeUntil( registry, () => registry.select( MODULES_ADSENSE ).hasFinishedResolution( 'getOriginalAccountStatus' ) );
			...
			it( 'does not override original account status when receiving settings again', async () => {
				...
				registry.dispatch( MODULES_ADSENSE ).receiveGetSettings( { accountStatus: 'another-status' } );
				expect( registry.select( MODULES_ADSENSE ).getOriginalAccountStatus() ).toEqual( value );
			} );
		} );
	} );
