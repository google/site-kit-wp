...
export default function SetupMain( { finishSetup } ) {
	...
	const previousAccountStatus = useSelect( ( select ) =>
		select( STORE_NAME ).getAccountStatus()
	);
	const previousSiteStatus = useSelect( ( select ) =>
		select( STORE_NAME ).getSiteStatus()
	);
	const accountSetupComplete = useSelect( ( select ) =>
		select( STORE_NAME ).getAccountSetupComplete()
	);
	const siteSetupComplete = useSelect( ( select ) =>
		select( STORE_NAME ).getSiteSetupComplete()
	);
	...
	const clients = useSelect( ( select ) =>
		select( STORE_NAME ).getClients( accountID )
	);
	...
	const alerts = useSelect( ( select ) =>
		select( STORE_NAME ).getAlerts( accountID )
	);
	const urlChannels = useSelect( ( select ) =>
		select( STORE_NAME ).getURLChannels( accountID, clientID )
	);
	const urlChannelsError = useSelect( ( select ) =>
		select( STORE_NAME ).getErrorForSelector( 'getURLChannels', [
			accountID,
			clientID,
		] )
	);
	const accountsError = useSelect( ( select ) =>
		select( STORE_NAME ).getError( 'getAccounts', [] )
	);
	const alertsError = useSelect( ( select ) =>
		select( STORE_NAME ).getError( 'getAlerts', [ accountID ] )
	);
	...
	const isAdBlockerActive = useSelect( ( select ) =>
		select( STORE_NAME ).isAdBlockerActive()
	);
