...
describe( 'ShowNextToCommentsToggle', () => {
	...
	it( 'should show the "Anyone can register" message when anyoneCanRegister is false on a single site', () => {
		registry
			.dispatch( MODULES_SIGN_IN_WITH_GOOGLE )
			.receiveGetSettings( validSettings );
		registry.dispatch( CORE_SITE ).receiveSiteInfo( {
		...
		const { container } = render( <ShowNextToCommentsToggle />, {
			registry,
		} );
	...
	it( 'should show the "Allow new registrations" message when anyoneCanRegister is false on a multisite', () => {
		registry
			.dispatch( MODULES_SIGN_IN_WITH_GOOGLE )
			.receiveGetSettings( validSettings );
		registry.dispatch( CORE_SITE ).receiveSiteInfo( {
	...
	it( 'should not show the registration message when anyoneCanRegister is true', () => {
		registry
			.dispatch( MODULES_SIGN_IN_WITH_GOOGLE )
			.receiveGetSettings( validSettings );
		registry.dispatch( CORE_SITE ).receiveSiteInfo( {
		...
		expect( container.textContent ).not.toContain(
			'Requires open user registration'
		);
	} );
} );
