	...
	describe( 'selectors', () => {
		describe( 'getServiceURL', () => {
			it( 'retrieves the correct URL with no arguments', async () => {
				const serviceURL = registry.select( MODULES_OPTIMIZE ).getServiceURL();
			...
			it( 'adds the path parameter', () => {
				...
				const serviceURLNoSlashes = registry.select( MODULES_OPTIMIZE ).getServiceURL( { path: 'test/path/to/deeplink' } );
				expect( serviceURLNoSlashes ).toEqual( expectedURL );
				const serviceURLWithLeadingSlash = registry.select( MODULES_OPTIMIZE ).getServiceURL( { path: '/test/path/to/deeplink' } );
			...
			it( 'adds query args', async () => {
				...
				const serviceURL = registry.select( MODULES_OPTIMIZE ).getServiceURL( { path, query } );
