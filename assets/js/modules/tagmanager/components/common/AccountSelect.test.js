...
describe( 'AccountSelect', () => {
	...
	beforeEach( () => {
		...
		registry.dispatch( MODULES_TAGMANAGER ).receiveGetContainers( [], { accountID: fixtures.accounts[ 0 ].accountId } );
	...
	it( 'should update accountID in the store when a new item is clicked', async () => {
		...
		const originalAccountID = registry.select( MODULES_TAGMANAGER ).getAccountID();
		...
		const newAccountID = registry.select( MODULES_TAGMANAGER ).getAccountID();
