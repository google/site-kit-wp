...
export default function AMPContainerSelect() {
	...
	const ampContainers = useSelect( ( select ) => select( MODULES_TAGMANAGER ).getAMPContainers( accountID ) );
	const isAMP = useSelect( ( select ) => select( CORE_SITE ).isAMP() );
	const isSecondaryAMP = useSelect( ( select ) => select( CORE_SITE ).isSecondaryAMP() );

	const { setAMPContainerID, setInternalAMPContainerID } = useDispatch( MODULES_TAGMANAGER );
