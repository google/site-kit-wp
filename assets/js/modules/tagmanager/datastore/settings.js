...
export async function submitChanges( { select, dispatch } ) {
	...
	if ( containerID === CONTAINER_CREATE ) {
		...
		await dispatch( STORE_NAME ).setContainerID( container.publicId );
		await dispatch( STORE_NAME ).setInternalContainerID(
			// eslint-disable-next-line sitekit/acronym-case
	...
	if ( ampContainerID === CONTAINER_CREATE ) {
		...
		// eslint-disable-next-line sitekit/acronym-case
		await dispatch( STORE_NAME ).setAMPContainerID( container.publicId );
		await dispatch( STORE_NAME ).setInternalAMPContainerID(
			// eslint-disable-next-line sitekit/acronym-case
			container.containerId
		);
	}
