	...
	describe( 'selectors', () => {
		describe( 'getDraftPostIdeas', () => {
			...
			it( 'uses a resolver to make a network request', async () => {
				...
				const pendingDraftPostIdeas = registry.select( MODULES_IDEA_HUB ).getDraftPostIdeas( options );

				expect( pendingDraftPostIdeas ).toBeUndefined();
				await untilResolved( registry, MODULES_IDEA_HUB ).getDraftPostIdeas( options );
			...
			it( 'uses offset and length parameters to adjust/limit the ideas returned by the selector', async () => {
				...
				registry.select( MODULES_IDEA_HUB ).getDraftPostIdeas( customOptions );
				await untilResolved( registry, MODULES_IDEA_HUB ).getDraftPostIdeas( customOptions );

				const draftPostIdeas = registry.select( MODULES_IDEA_HUB ).getDraftPostIdeas( customOptions );
			...
			it( 'only fetches once even with different options are passed', async () => {
				...
				registry.select( MODULES_IDEA_HUB ).getDraftPostIdeas( customOptions );
				registry.select( MODULES_IDEA_HUB ).getDraftPostIdeas( options );
			...
			it( 'does not make a network request if report for given options is already present', async () => {
				...
				registry.dispatch( MODULES_IDEA_HUB ).receiveGetDraftPostIdeas( fixtures.draftPostIdeas, { options } );

				const report = registry.select( MODULES_IDEA_HUB ).getDraftPostIdeas( options );
