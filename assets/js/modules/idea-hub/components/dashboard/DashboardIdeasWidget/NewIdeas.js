...
const NewIdeas = ( { WidgetReportError } ) => {
	...
	const totalNewIdeas = useSelect(
		( select ) => select( STORE_NAME ).getNewIdeas()?.length
	);
	const newIdeas = useSelect( ( select ) =>
		select( STORE_NAME ).getNewIdeas( args )
	);
	const hasFinishedResolution = useSelect( ( select ) =>
		select( STORE_NAME ).hasFinishedResolution( 'getNewIdeas', [ args ] )
	);
	const error = useSelect( ( select ) =>
		select( STORE_NAME ).getErrorForSelector( 'getNewIdeas', [ args ] )
	);
