	...
	describe( 'actions', () => {
		describe( 'createProperty', () => {
			it( 'creates a property and adds it to the store ', async () => {
				...
				fetch
					.doMockIf(
						/^\/google-site-kit\/v1\/modules\/search-console\/data\/create-property/
					)
					.mockResponse(
			...
			it( 'dispatches an error if the request fails ', async () => {
				...
				fetch
					.doMockIf(
						/^\/google-site-kit\/v1\/modules\/search-console\/data\/properties-profiles/
					)
		...
		describe( 'getProperties', () => {
			...
			it( 'does not make a network request if properties for this account are already present', async () => {
				...
				expect( fetch ).not.toHaveBeenCalledWith(
					expect.stringContaining( '/google-site-kit/v1/modules/search-console/data/properties-profiles?' ),
					expect.any( Object )
				);
				expect( properties ).toEqual( fixtures.propertiesProfiles.properties );
				expect( properties ).toHaveLength( 17 );
