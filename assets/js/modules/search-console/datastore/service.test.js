	...
	describe( 'selectors', () => {
		describe( 'getServiceURL', () => {
			it( 'retrieves the correct URL with no arguments', async () => {
				const serviceURL = registry.select( MODULES_SEARCH_CONSOLE ).getServiceURL();
			...
			it( 'appends the given path (without leading slash) to the path of the base URL', () => {
				const serviceURL = registry.select( MODULES_SEARCH_CONSOLE ).getServiceURL( { path: 'test/path/to/deeplink' } );
			...
			it( 'merges given query args to the base service URL args', async () => {
				const foo = 'bar';
				const baz = 'buzz';
				const serviceURL = registry.select( MODULES_SEARCH_CONSOLE ).getServiceURL( { query: { foo, baz } } );
				expect( serviceURL.startsWith( baseURI ) ).toBe( true );
			...
			it( 'does not take precedence over the authuser query arg', () => {
				...
				const serviceURL = registry.select( MODULES_SEARCH_CONSOLE ).getServiceURL( { query } );
				expect( serviceURL.startsWith( baseURI ) ).toBe( true );
		...
		describe( 'getServiceReportURL', () => {
			beforeEach( () => {
				registry.dispatch( MODULES_SEARCH_CONSOLE ).setPropertyID( propertyID );
			} );

			it( 'returns a deep link to a search-analytics report', () => {
				const serviceURL = registry.select( MODULES_SEARCH_CONSOLE ).getServiceReportURL();
			...
			it( 'sets a default `page` query arg for domain properties', () => {
				registry.dispatch( MODULES_SEARCH_CONSOLE ).setPropertyID( domainPropertyID );
				expect( registry.select( MODULES_SEARCH_CONSOLE ).isDomainProperty() ).toBe( true );
				const serviceURL = registry.select( MODULES_SEARCH_CONSOLE ).getServiceReportURL();
			...
			it( 'sets a default `page` query arg for domain properties that takes precedence over an `undefined` page report arg', () => {
				registry.dispatch( MODULES_SEARCH_CONSOLE ).setPropertyID( domainPropertyID );
				expect( registry.select( MODULES_SEARCH_CONSOLE ).isDomainProperty() ).toBe( true );
				const serviceURL = registry.select( MODULES_SEARCH_CONSOLE ).getServiceReportURL( { page: undefined } );
				const referenceSiteURL = registry.select( CORE_SITE ).getReferenceSiteURL();
				expect( serviceURL ).toMatchQueryParameters( {
					resource_id: domainPropertyID,
					page: `*${ referenceSiteURL }`,
		...
		describe( 'isDomainProperty', () => {
			it( 'should identify if property is search console domain property', async () => {
				registry.dispatch( MODULES_SEARCH_CONSOLE ).setPropertyID( propertyID );
				expect( registry.select( MODULES_SEARCH_CONSOLE ).isDomainProperty() ).toBe( false );
