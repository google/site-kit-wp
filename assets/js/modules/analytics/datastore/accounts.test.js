	...
	describe( 'actions', () => {
		describe( 'createAccount', () => {
			...
			it( 'sets isDoingCreateAccount ', async () => {
				...
				expect( registry.select( MODULES_ANALYTICS ).isDoingCreateAccount() ).toEqual( true );
			} );
			...
			it( 'dispatches an error if the request fails ', async () => {
				...
				expect( registry.select( MODULES_ANALYTICS ).getErrorForAction( 'createAccount' ) ).toMatchObject( response );
		...
		describe( 'resetAccounts', () => {
			it( 'sets accounts and related values back to their initial values', async () => {
				...
				registry.dispatch( MODULES_ANALYTICS ).receiveGetProperties( fixtures.accountsPropertiesProfiles.properties, { accountID } );
				registry.dispatch( MODULES_ANALYTICS ).receiveGetProfiles( fixtures.accountsPropertiesProfiles.profiles, { accountID, propertyID } );
				...
				expect( registry.select( MODULES_ANALYTICS ).getUseSnippet() ).toStrictEqual( true );
				expect( registry.select( MODULES_ANALYTICS ).getTrackingDisabled() ).toStrictEqual( [] );
				expect( registry.select( MODULES_ANALYTICS ).getAnonymizeIP() ).toStrictEqual( true );
				// Wait until selector is resolved to prevent unmatched fetch error.
				await subscribeUntil( registry, () => registry.select( MODULES_ANALYTICS )
			...
			it( 'invalidates the resolver for getAccounts', async () => {
				...
				await subscribeUntil(
					registry,
					() => registry.select( MODULES_ANALYTICS ).hasFinishedResolution( 'getAccounts' ),
				);

				registry.dispatch( MODULES_ANALYTICS ).resetAccounts();

				expect( registry.select( MODULES_ANALYTICS ).hasFinishedResolution( 'getAccounts' ) ).toStrictEqual( false );
			} );
		} );
		...
		describe( 'selectAccount', () => {
			...
			it( 'should throw an error if accountID is invalid', () => {
				expect( () => registry.dispatch( MODULES_ANALYTICS ).selectAccount( false ) ).toThrow();
			} );

			it( 'should property reset propertyID and profileID when selecting ACCOUNT_CREATE option', () => {
				registry.dispatch( MODULES_ANALYTICS ).selectAccount( ACCOUNT_CREATE );
				expect( registry.select( MODULES_ANALYTICS ).getAccountID() ).toBe( ACCOUNT_CREATE );
				expect( registry.select( MODULES_ANALYTICS ).getPropertyID() ).toBe( '' );
				expect( registry.select( MODULES_ANALYTICS ).getInternalWebPropertyID() ).toBe( '' );
			...
			it( 'should correctly select property and profile IDs', async () => {
				...
				expect( registry.select( MODULES_ANALYTICS ).getAccountID() ).toBe( accountID );
				expect( registry.select( MODULES_ANALYTICS ).getPropertyID() ).toBe( fixtures.propertiesProfiles.profiles[ 0 ].webPropertyId ); // eslint-disable-line sitekit/acronym-case
				expect( registry.select( MODULES_ANALYTICS ).getInternalWebPropertyID() ).toBe( fixtures.propertiesProfiles.profiles[ 0 ].internalWebPropertyId ); // eslint-disable-line sitekit/acronym-case
				expect( registry.select( MODULES_ANALYTICS ).getProfileID() ).toBe( fixtures.propertiesProfiles.profiles[ 0 ].id );
			} );
			...
			describe( 'analytics-4', () => {
				...
				it( 'should select the correct UA property', async () => {
					...
					await registry.dispatch( MODULES_ANALYTICS ).selectAccount( accountID );
					expect( registry.select( MODULES_ANALYTICS ).getPropertyID() ).toBe( fixtures.propertiesProfiles.properties[ 0 ].id );
				} );
	...
	describe( 'selectors', () => {
		describe( 'getAccounts', () => {
			...
			it( 'uses a resolver to make a network request', async () => {
				...
				const initialAccounts = registry.select( MODULES_ANALYTICS ).getAccounts();

				expect( initialAccounts ).toEqual( undefined );
				await subscribeUntil( registry,
					() => (
						registry.select( MODULES_ANALYTICS ).getAccounts() !== undefined
					),
				);
			...
			it( 'passes existing tag ID when fetching accounts', async () => {
				...
				registry.dispatch( MODULES_ANALYTICS ).receiveGetExistingTag( existingPropertyID );
				...
				await subscribeUntil( registry,
					() => registry.select( MODULES_ANALYTICS ).getAccounts() !== undefined ||
					registry.select( MODULES_ANALYTICS ).getErrorForSelector( 'getAccounts' ),
				);
			...
			it( 'sets account, property, and profile IDs in the store, if a matchedProperty is received and an account is not selected yet', async () => {
				...
				expect( registry.select( MODULES_ANALYTICS ).getAccountID() ).toBeFalsy();
				expect( registry.select( MODULES_ANALYTICS ).getPropertyID() ).toBeFalsy();
				expect( registry.select( MODULES_ANALYTICS ).getInternalWebPropertyID() ).toBeFalsy();
				expect( registry.select( MODULES_ANALYTICS ).getProfileID() ).toBeFalsy();
				...
				expect( registry.select( MODULES_ANALYTICS ).getAccountID() ).toBe( matchedProperty.accountId ); // eslint-disable-line sitekit/acronym-case
				expect( registry.select( MODULES_ANALYTICS ).getPropertyID() ).toBe( matchedProperty.id );
				expect( registry.select( MODULES_ANALYTICS ).getInternalWebPropertyID() ).toBe( matchedProperty.internalWebPropertyId ); // eslint-disable-line sitekit/acronym-case
				expect( registry.select( MODULES_ANALYTICS ).getProfileID() ).toBe( matchedProperty.defaultProfileId ); // eslint-disable-line sitekit/acronym-case
			} );
		...
		describe( 'getAccountTicketTermsOfServiceURL', () => {
			it( 'requires the accountTicketID from createAccount', () => {
				...
				expect( registry.select( MODULES_ANALYTICS ).getAccountTicketTermsOfServiceURL() ).toEqual( undefined );

				registry.dispatch( MODULES_ANALYTICS ).receiveCreateAccount( { id: 'test-account-ticket-id' }, { data: {} } );

				expect( registry.select( MODULES_ANALYTICS ).getAccountTicketTermsOfServiceURL() ).toEqual( 'https://analytics.google.com/analytics/web/?provisioningSignup=false&authuser=test%40gmail.com#/termsofservice/test-account-ticket-id' );
			} );
