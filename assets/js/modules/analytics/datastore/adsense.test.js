	...
	describe( 'actions', () => {
		describe( 'setAdsenseLinked', () => {
			it.each( [
				...
				registry.dispatch( MODULES_ANALYTICS ).setAdsenseLinked( input );

				expect( store.getState().adsenseLinked ).toBe( expected );
			} );

			it( 'is independent of the adsenseLinked setting', () => {
				registry.dispatch( MODULES_ANALYTICS ).receiveGetSettings( { adsenseLinked: false } );
	...
	describe( 'selectors', () => {
		describe( 'getAdsenseLinked', () => {
			it( 'resolves the initial value from the adsenseLinked setting', async () => {
				...
				expect( registry.select( MODULES_ANALYTICS ).getAdsenseLinked() ).toBeUndefined();
			...
			it( 'supports asynchronous settings resolution', async () => {
				...
				const selectPromise = registry.__experimentalResolveSelect( MODULES_ANALYTICS ).getAdsenseLinked();
