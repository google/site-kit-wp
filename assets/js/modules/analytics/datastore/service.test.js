...
describe( 'module/analytics service store', () => {
	...
	beforeAll( () => {
		...
		registry.dispatch( MODULES_ANALYTICS ).receiveGetSettings( fixtures.settings.default );
	} );
	...
	describe( 'selectors', () => {
		describe( 'getServiceURL', () => {
			it( 'retrieves the correct URL with no arguments', async () => {
				const serviceURL = registry.select( MODULES_ANALYTICS ).getServiceURL();
			...
			it( 'adds the path parameter', () => {
				...
				const serviceURLNoSlashes = registry.select( MODULES_ANALYTICS ).getServiceURL( { path: 'test/path/to/deeplink' } );
				expect( serviceURLNoSlashes ).toEqual( expectedURL );
				const serviceURLWithLeadingSlash = registry.select( MODULES_ANALYTICS ).getServiceURL( { path: '/test/path/to/deeplink' } );
			...
			it( 'adds query args', async () => {
				...
				const serviceURL = registry.select( MODULES_ANALYTICS ).getServiceURL( { path, query } );
		...
		describe( 'getServiceReportURL', () => {
			...
			it( 'requires a report type', () => {
				expect( () => registry.select( MODULES_ANALYTICS ).getServiceReportURL() )
			...
			it( 'returns `undefined` when no accountID is set', () => {
				...
				expect( registry.select( MODULES_ANALYTICS ).getServiceReportURL( type ) ).toBeUndefined();
			} );
			...
			it( 'returns `undefined` when no profileID is set', () => {
				registry.dispatch( MODULES_ANALYTICS ).setAccountID( '12345' );
				registry.dispatch( MODULES_ANALYTICS ).setInternalWebPropertyID( '34567' );
			...
			describe( 'with necessary account data', () => {
				...
				beforeEach( () => {
					...
					registry.dispatch( MODULES_ANALYTICS ).setInternalWebPropertyID( internalWebPropertyID );
				...
				it( 'returns a service URL for the given report type for the current account, property, and profile', () => {
					const reportServiceURL = registry.select( MODULES_ANALYTICS ).getServiceReportURL( type );
				...
				it( 'returns a service URL for the given report type including any extra report-specific arguments', () => {
					...
					const reportServiceURL = registry.select( MODULES_ANALYTICS ).getServiceReportURL( type, reportArgs );
					const url = new URL( reportServiceURL );
					expect( reportServiceURL.startsWith( baseURI ) ).toBe( true );
