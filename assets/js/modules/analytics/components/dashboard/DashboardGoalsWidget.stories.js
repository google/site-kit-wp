...
NoGoalsSet.storyName = 'No Goals Set (CTA)';
NoGoalsSet.args = {
	setupRegistry: ( { dispatch } ) => {
		for ( const options of reportOptions ) {
			const report = getAnalyticsMockResponse( options );

			dispatch( MODULES_ANALYTICS ).receiveGetReport( report, {
				options,
			} );
		}

		const emptyGoals = { ...goals, items: [] };

		dispatch( MODULES_ANALYTICS ).receiveGetGoals( emptyGoals );
	},
};
NoGoalsSet.parameters = {
	features: [ 'zeroDataStates' ],
};

export const NoGoalsSetViewOnly = Template.bind( {} );
NoGoalsSetViewOnly.storyName = 'No Goals Set (View-only)';
NoGoalsSetViewOnly.args = {
	setupRegistry: ( { dispatch } ) => {
		for ( const options of reportOptions ) {
			const report = getAnalyticsMockResponse( options );
...
GatheringDataZeroDataStates.storyName = 'Gathering Data';
