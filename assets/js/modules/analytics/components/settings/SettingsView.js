...
export default function SettingsView() {
	...
	const internalWebPropertyID = useSelect( ( select ) =>
		select( STORE_NAME ).getInternalWebPropertyID()
	);
	...
	const anonymizeIP = useSelect( ( select ) =>
		select( STORE_NAME ).getAnonymizeIP()
	);
	const trackingDisabled =
		useSelect( ( select ) => select( STORE_NAME ).getTrackingDisabled() ) ||
		[];
	...
	const editViewSettingsURL = useSelect( ( select ) =>
		select( STORE_NAME ).getServiceURL( {
