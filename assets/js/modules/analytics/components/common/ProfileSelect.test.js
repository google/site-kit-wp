...
const setupRegistry = ( { dispatch } ) => {
	...
	dispatch( MODULES_ANALYTICS ).receiveGetProfiles( fixtures.propertiesProfiles.profiles, { accountID, propertyID } );
...
const setupRegistryWithExistingTag = ( { dispatch } ) => {
	...
	dispatch( MODULES_ANALYTICS ).setProfileID( id );
	dispatch( MODULES_ANALYTICS ).receiveGetProperties( fixtures.accountsPropertiesProfiles.properties, { accountID: existingTag.accountID } );

	dispatch( MODULES_ANALYTICS ).receiveGetAccounts( fixtures.accountsPropertiesProfiles.accounts );
	dispatch( MODULES_ANALYTICS ).finishResolution( 'getAccounts', [] );

	dispatch( MODULES_ANALYTICS ).finishResolution( 'getProperties', [ existingTag.accountID ] );
	dispatch( MODULES_ANALYTICS ).receiveGetProfiles( fixtures.accountsPropertiesProfiles.profiles, { accountID: existingTag.accountID, propertyID: existingTag.propertyID } );

	dispatch( MODULES_ANALYTICS ).finishResolution( 'getProfiles', [ existingTag.accountID, existingTag.propertyID ] );
	dispatch( MODULES_ANALYTICS ).receiveGetExistingTag( existingTag.propertyID );
};
...
const setupEmptyRegistry = ( { dispatch } ) => {
	...
	dispatch( MODULES_ANALYTICS ).receiveGetProfiles( [], { accountID, propertyID } );
	dispatch( MODULES_ANALYTICS ).finishResolution( 'getProfiles', [ accountID, propertyID ] );
};
...
describe( 'ProfileSelect', () => {
	...
	it( 'should display profile options of an existing account when present, and not be disabled.', async () => {
		...
		const currentPropertyID = registry.select( MODULES_ANALYTICS ).getPropertyID();
		const existingTagPropertyID = registry.select( MODULES_ANALYTICS ).getExistingTag();
	...
	it( 'should not render if account ID is not valid', async () => {
		const { container, registry } = render( <ProfileSelect />, {
			setupRegistry( { dispatch } ) {
				setupRegistry( { dispatch } );
				dispatch( MODULES_ANALYTICS ).finishResolution( 'getProperties', [ '0' ] );
			},
		} );
		...
		await act( () => registry.dispatch( MODULES_ANALYTICS ).setAccountID( '0' ) );

		// An empty accountID is invalid, so ensure the select is not rendered.
		expect( container ).toBeEmptyDOMElement();

		// eslint-disable-next-line sitekit/acronym-case
		await act( () => registry.dispatch( MODULES_ANALYTICS ).setAccountID( fixtures.propertiesProfiles.profiles[ 0 ].accountId ) );
	...
	it( 'should not render if account ID or property ID are invalid', async () => {
		...
		const validAccountID = registry.select( MODULES_ANALYTICS ).getAccountID();
		const validPropertyID = registry.select( MODULES_ANALYTICS ).getPropertyID();

		// A valid accountID is provided, so the select component should not be disabled.
		expect( container.querySelector( '.googlesitekit-analytics__select-profile' ) )
			.not.toHaveClass( 'mdc-select--disabled' );

		await act( () => registry.dispatch( MODULES_ANALYTICS ).setAccountID( validAccountID ) );
		await act( () => registry.dispatch( MODULES_ANALYTICS ).setPropertyID( '0' ) );

		// The accountID is valid, but an empty propertyID is invalid, so ensure the select is not rendered.
		expect( container ).toBeEmptyDOMElement();

		await act( () => registry.dispatch( MODULES_ANALYTICS ).setPropertyID( validPropertyID ) );
	...
	it( 'should update profileID in the store when a new item is selected', async () => {
		...
		const originalProfileID = registry.select( MODULES_ANALYTICS ).getProfileID();
