...
const SET_RESOURCE_PARTIAL_DATA_STATE = 'SET_RESOURCE_PARTIAL_DATA_STATE';
...
const baseActions = {
	...
	/**
	 * Sets the partial data state for a specific resource.
	 *
	 * @since n.e.x.t
	 *
	 * @param {string}  resourceSlug  Resource slug.
	 * @param {string}  resourceType  Resource type.
	 * @param {boolean} isPartialData TRUE if partial data, otherwise FALSE.
	 * @return {Object} Redux-style action.
	 */
	setResourcePartialDataState( resourceSlug, resourceType, isPartialData ) {
		invariant(
			'string' === typeof resourceSlug && resourceSlug.length > 0,
			'resourceSlug must be a non-empty string.'
		);
		...
		invariant(
			'boolean' === typeof isPartialData,
			'isPartialData must be a boolean.'
		);

		return {
			payload: { resourceSlug, resourceType, isPartialData },
			type: SET_RESOURCE_PARTIAL_DATA_STATE,
		};
	},
};
	...
	*getResourceDataAvailabilityDates() {
		...
		if (
			registry
				...
				.getResourceDataAvailabilityDates() !== undefined
		) {
			return;
		}
...
const baseSelectors = {
	...
	isResourcePartialData: createRegistrySelector(
		( select ) => ( state, resourceSlug, resourceType ) => {
			...
			const startDateinMS = stringToDate( startDate ).getTime();

			// If the data availability date is same or earlier than the date range start date,
			// the resource is not in partial data state.
			return dataAvailabilityDate <= startDateinMS;
		}
	),
