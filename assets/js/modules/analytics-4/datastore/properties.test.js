	...
	describe( 'actions', () => {
		...
		describe( 'selectProperty', () => {
			...
			it( 'should set module settings correctly when PROPERTY_CREATE is passed', async () => {
				...
				expect( registry.select( MODULES_ANALYTICS_4 ).getWebDataStreamID() ).toBe( WEBDATASTREAM_CREATE );
			...
			it( 'should set property ID only and reset datastream with measurement IDs when web data stream is not found', async () => {
				...
				await registry.dispatch( MODULES_ANALYTICS_4 ).selectProperty( propertyID );
			...
			it( 'should set property, datastream, and measurement IDs when web data stream is found', async () => {
				...
				expect( registry.select( MODULES_ANALYTICS_4 ).getPropertyID() ).toBe( propertyID );
				expect( registry.select( MODULES_ANALYTICS_4 ).getWebDataStreamID() ).toBe( fixtures.webDataStreams[ 1 ]._id );
				expect( registry.select( MODULES_ANALYTICS_4 ).getMeasurementID() ).toBe( fixtures.webDataStreams[ 1 ].measurementId ); // eslint-disable-line sitekit/acronym-case
			} );
			...
			it( 'supports asynchronous webdatastream resolution', async () => {
				...
				const promise = registry.dispatch( MODULES_ANALYTICS_4 ).selectProperty( propertyID );
		...
		describe( 'matchAccountProperty', () => {
			...
			beforeEach( () => {
				...
				registry.dispatch( MODULES_ANALYTICS_4 ).receiveGetProperties( properties, { accountID } );
			} );
			...
			it( 'should return NULL if no property matches the current site', async () => {
				...
				const property = await registry.dispatch( MODULES_ANALYTICS_4 ).matchAccountProperty( accountID );
		...
		describe( 'matchAndSelectProperty', () => {
			...
			beforeEach( () => {
				...
				registry.dispatch( MODULES_ANALYTICS_4 ).receiveGetProperties( properties, { accountID } );
				registry.dispatch( MODULES_ANALYTICS_4 ).receiveGetWebDataStreamsBatch( webDataStreams, { propertyIDs: Object.keys( webDataStreams ) } );
			} );
			...
			it( 'should select the fallback property if the matching property is not found', async () => {
				...
				await registry.dispatch( MODULES_ANALYTICS_4 ).matchAndSelectProperty( accountID, PROPERTY_CREATE );

				expect( registry.select( MODULES_ANALYTICS_4 ).getSettings() ).toMatchObject( {
			...
			it( 'should select the correct property ID if we can find a matching property', async () => {
				await registry.dispatch( MODULES_ANALYTICS_4 ).matchAndSelectProperty( accountID );
		...
		describe( 'matchPropertyByURL', () => {
			...
			beforeEach( () => {
				registry.dispatch( MODULES_ANALYTICS_4 ).receiveGetProperty( property, { propertyID } );
				registry.dispatch( MODULES_ANALYTICS_4 ).receiveGetWebDataStreamsBatch( fixtures.webDataStreamsBatch, { propertyIDs } );
			} );

			it( 'should return a property object when a property is found', async () => {
				const url = 'https://www.example.org/';
				const matchedProperty = await registry.dispatch( MODULES_ANALYTICS_4 ).matchPropertyByURL( propertyIDs, url );
		...
		describe( 'matchPropertyByMeasurementID', () => {
			...
			it( 'should return a property object when a property is found', async () => {
				...
				const matchedProperty = await registry.dispatch( MODULES_ANALYTICS_4 ).matchPropertyByMeasurementID( propertyIDs, measurementID );
				expect( matchedProperty ).toEqual( property );
			} );
	...
	describe( 'selectors', () => {
		describe( 'getProperties', () => {
			it( 'should use a resolver to make a network request', async () => {
				...
				await untilResolved( registry, MODULES_ANALYTICS_4 ).getProperties( accountID );
			...
			it( 'should not make a network request if properties for this account are already present', async () => {
				...
				await untilResolved( registry, MODULES_ANALYTICS_4 ).getProperties( testAccountID );
			...
			it( 'should dispatch an error if the request fails', async () => {
				...
				await untilResolved( registry, MODULES_ANALYTICS_4 ).getProperties( fakeAccountID );
				expect( fetchMock ).toHaveFetchedTimes( 1 );
		...
		describe( 'getProperty', () => {
			it( 'should use a resolver to make a network request', async () => {
				...
				const initialProperty = registry.select( MODULES_ANALYTICS_4 ).getProperty( propertyID );
				expect( initialProperty ).toBeUndefined();

				await untilResolved( registry, MODULES_ANALYTICS_4 ).getProperty( propertyID );
			...
			it( 'should not make a network request if the property is already present', async () => {
				const propertyID = '12345';
				registry.dispatch( MODULES_ANALYTICS_4 ).receiveGetProperty( fixtures.properties[ 0 ], { propertyID } );
			...
			it( 'should not make a network request if the property is already received via getProperties selector', async () => {
				...
				registry.select( MODULES_ANALYTICS_4 ).getProperties( accountID );
				await untilResolved( registry, MODULES_ANALYTICS_4 ).getProperties( accountID );
			...
			it( 'should dispatch an error if the request fails', async () => {
				...
				registry.select( MODULES_ANALYTICS_4 ).getProperty( propertyID );
				await untilResolved( registry, MODULES_ANALYTICS_4 ).getProperty( propertyID );
				expect( fetchMock ).toHaveFetchedTimes( 1 );
