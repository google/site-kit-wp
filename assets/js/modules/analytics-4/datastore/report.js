...
import { isValidMetrics } from '../util/report-validation';
...
const fetchGetReportStore = createFetchStore( {
	...
	controlCallback: ( { options } ) => {
		return API.get( 'modules', 'analytics-4', 'report', normalizeReportOptions( options ) );
	},
	...
	validateParams: ( { options } = {} ) => {
		...
		invariant(
			...
			'Metrics for an Analytics report must be either a string, an array of strings, an object, an array of objects or a mix of strings and objects. If an object is used, it must have "expression" and "name" properties.',
		);

		// TODO: Add support for dimensions and orderbys.
	},
} );
