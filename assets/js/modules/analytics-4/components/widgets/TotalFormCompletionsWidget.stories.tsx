/**
 * TotalFormCompletionsWidget component stories.
 *
...
// The detected lead events, in the fixed order `getDetectedLeadEvents`
// returns them (filtered from `CONVERSION_REPORTING_LEAD_EVENTS`).
...
// Non-null assertion is safe: a literal, always-defined events array is passed.
...
interface TotalFormCompletionsWidgetStoryArgs {
	setupRegistry: (
...
function Template( {
	setupRegistry,
	...args
}: TotalFormCompletionsWidgetStoryArgs ) {
	return (
...
export default {
	title: 'Key Metrics/TotalFormCompletionsWidget',
	decorators: [
		(
			Story: ComponentType,
			{ args }: { args: TotalFormCompletionsWidgetStoryArgs }
		) => {
			function setupRegistry(
				...
				registry
					.dispatch( MODULES_ANALYTICS_4 )
					.setDetectedEvents( detectedLeadEvents );
