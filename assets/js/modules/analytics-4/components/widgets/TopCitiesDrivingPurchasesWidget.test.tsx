/**
 * TopCitiesDrivingPurchasesWidget component tests.
 *
...
describe( 'TopCitiesDrivingPurchasesWidget', () => {
	...
	it( 'should render the loading state while resolving the report', async () => {
		...
		const { container, waitForRegistry } = render(
			<TopCitiesDrivingPurchasesWidget { ...widgetProps } />,
			{ registry }
		);
	...
	it( "should render each row's share of the total as a percentage", async () => {
	...
	it( 'should show only the top three of the six rows it fetches', async () => {
		...
		// 60 of the 210 fetched, not of the 150 shown: the percentages have to
		// cover every row fetched or they would not match the Site Goals tile.
		expect( getByText( '28.6%' ) ).toBeInTheDocument();
	} );
} );
