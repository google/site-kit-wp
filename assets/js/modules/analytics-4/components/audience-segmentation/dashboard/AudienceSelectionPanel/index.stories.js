...
export const UserCountErrorForSKAudience = Template.bind( {} );
UserCountErrorForSKAudience.storyName =
	'User count retrieval error when SK audience is in partial data state';
UserCountErrorForSKAudience.args = {
	setupRegistry: ( registry ) => {
		const error = {
			code: 'test_error',
			message: 'Error message.',
		...
		const { getSiteKitAudiencesUserCountReportOptions } =
			registry.select( MODULES_ANALYTICS_4 );
		...
		const otherAudiences = availableAudiences.filter(
			( { audienceType } ) => 'SITE_KIT_AUDIENCE' !== audienceType
		);

		const otherAudiencesReportOptions = {
			...baseReportOptions,
		...
		provideAnalytics4MockReport( registry, otherAudiencesReportOptions );
...
UserCountErrorForSKAudience.scenario = {};
