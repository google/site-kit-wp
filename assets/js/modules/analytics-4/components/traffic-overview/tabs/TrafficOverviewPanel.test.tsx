...
describe( 'TrafficOverviewPanel', () => {
	...
	it( 'renders the three sections as placeholders while one of the five reports is still loading', async () => {
		...
		registry
			.dispatch( MODULES_ANALYTICS_4 )
			.receiveGetReport( createTotalsReport( 1200, 1000 ), {
				options: totalsArgs,
			} );
		registry
			.dispatch( MODULES_ANALYTICS_4 )
			.receiveGetReport(
				createDailyVisitorsReport( [ [ '2025-01-14', 40 ] ] ),
				{ options: graphArgs }
			);
		registry
			.dispatch( MODULES_ANALYTICS_4 )
			.receiveGetReport( createBreakdownReport( [ [ 'Direct', 700 ] ] ), {
				options: channelsArgs,
			} );
		registry
			.dispatch( MODULES_ANALYTICS_4 )
			.receiveGetReport(
				createBreakdownReport( [ [ 'Germany', 500 ] ] ),
		...
		expect(
			queryByText( 'January 14, 2025: 40 visitors' )
	...
	it( 'renders the visitor total, the chart and the three columns together when the last of the five reports arrives', async () => {
		...
		act( () => {
			registry
				.dispatch( MODULES_ANALYTICS_4 )
				.receiveGetReport(
					createBreakdownReport( [ [ 'desktop', 900 ] ] ),
					{ options: devicesArgs }
				);
			registry
				.dispatch( MODULES_ANALYTICS_4 )
	...
	it( 'renders the "Gathering data…" notice and three empty columns while the property is gathering data', async () => {
		...
		remainingArgs.forEach( ( options ) =>
			registry
				.dispatch( MODULES_ANALYTICS_4 )
				.receiveGetReport(
					createBreakdownReport( [ [ 'Direct', 700 ] ] ),
					{ options }
				)
		);
	...
	it( 'renders 0 visitors, a 0% badge and three empty columns when the reports show no traffic', async () => {
		...
		registry
			.dispatch( MODULES_ANALYTICS_4 )
			.receiveGetReport( createTotalsReport( 0, 0 ), {
				options: totalsArgs,
			} );
		remainingArgs.forEach( ( options ) =>
			registry
