...
7. **SKS** then redirects the user onwards:
    A. If the site already has received its site credentials:
    ...
    B. If the site does not yet have received its site credentials:
...
8. **User** navigates through the three required steps on the **SKS** setup endpoint for authenticating and receiving an OAuth token.
    * The three steps are "verification", "delegation consent", and "Search Console property" (see the section below for details on the steps). The user has to complete each step in order to successfully authenticate, which effectively for them just means to click the CTA button in each step, and Site Kit Service will take care of it.
    * As soon as the user is verified as a site owner (i.e. the "verification" step is complete), if the site does not yet have received its site credentials, that can happen now.
