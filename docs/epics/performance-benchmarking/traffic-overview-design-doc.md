...
3. **Traffic breakdown** — three columns, one per dimension: channels, locations and devices, each a ranked list of values with its own share of the column's total.
...
* **`getBreakdownReportArgs()` needs only the selected range, not a comparison one.** A breakdown row's value is a share of the column's own total rather than a change against the previous period, so the comparison range this builder also supports is left off the three breakdown reports and used only by `getTotalsReportOptions()` for the headline figure.  
...
The new work is the widget and its five components, one report hook, a pure helper that turns a breakdown report into ranked, percentage-of-total rows, one SCSS partial, and the flag. There is no new external dependency: the GA4 Data API is reached the way it always is.
...
| `totalUsers` by `sessionDefaultChannelGrouping` | `getBreakdownReportArgs()` | the channels column |
| `totalUsers` by `country` | `getBreakdownReportArgs()` | the locations column |
| `totalUsers` by `deviceCategory` | `getBreakdownReportArgs()` | the devices column |
...
`TrafficBreakdown` ([Figma](https://www.figma.com/design/MWN8TXAjfTeKLF0DZ91bIX/Performance-benchmarking?node-id=552-11543&m=dev)) renders three columns side by side — visitors by channels, by locations, by devices — each a ranked list of values with its share of the column's total per row. It is three components: `TrafficBreakdown` lays the columns out, `TrafficBreakdownColumn` renders one column's heading and rows, and `TrafficBreakdownRow` renders a label and a percentage.
...
**The rows are derived by a pure helper, not by the components.** `getBreakdownRows( report )` in `traffic-overview/utils/` takes a report over the selected range and returns `Array<{ label, percentage }>`, ordered by visitors descending. Two rules live in it:

* **The list is capped, and the remainder is aggregated.** Each column shows the top values and an "Others" row summing everything beneath them, at the same cap the donut applies today, so the migration does not change which values a reader sees at the top of a dimension. The "Others" row carries its own aggregated percentage, and unlike the donut slice it is not interactive, because nothing in this section is.  
...
**Each row's value is a share of the column's own total, not a raw count.** A row's percentage is its visitors divided by the sum across every row the report returns for that dimension, which is what keeps a column reading like a breakdown of one hundred percent rather than like an absolute count with nothing to compare it against.
...
* **The rows come from the panel's own helper.** `getBreakdownRows()` replaces `extractAnalyticsDataForPieChart` in the loader, so the printed rows are the rows on screen: the same ranking, the same cap, the same "Others" aggregate. The loader's `BreakdownRow` shape is unchanged — `{ label, percentage }` — because a column shows the same share of the total the donut legend showed; only the presentation moves from a ring to a ranked list.  
* **Three ranked tiles replace three donut tiles, unchanged.** `PDFMetricTileTable` is the existing tile for a ranked list and already renders a label and a metric per row with no per-row change, which is exactly what a breakdown column needs — no widening of the shared tile is required.  
...
Two existing files change outside the new directory, and they are the ones to call out at review: `assets/js/modules/analytics-4/widgets/index.js` gains the branch on the flag, and `assets/js/feature-tours/welcome.ts` gains the two retargeted selectors. `assets/js/components/pdf-export/shared-react-pdf-components/PDFMetricTileTable.tsx` needs no change: it already renders a label and a metric per row with no per-row change, which is exactly what a breakdown column needs.
...
We drop the donut. It shows one dimension, so a donut beside three columns is a fourth view of data already in the second one, and the two would have to agree about caps, ordering and the "Others" aggregate at all times. Both show the same share-of-total figure — the columns don't add a comparison the ring lacks — but a ring beside its own list is a redundant second encoding of one number, which is the cost this alternative would pay for keeping a visual anchor the columns already replace.
...
Report count is fixed at five and does not grow with the size of the site: every query is aggregated, the totals report carries its current and previous windows as two date ranges in one request, and the breakdown reports — each requesting only the selected range — are capped by GA4's own ordering rather than by row counts the browser has to filter. A site with 100k posts issues the same five requests a site with ten does.
...
Every string in the widget is a plugin string translated as usual. Numbers and percentages are formatted through `numFmt` throughout — the headline's change badge, the daily chart's axis, and the breakdown rows' percentages, including the "Others" row — so one convention applies across the card.
...
**The breakdown is the largest section issue and the one carrying real logic.** It owns three components, the column catalog, and `getBreakdownRows()` — where the ranking, the cap and the "Others" aggregate's own percentage are each wrong-but-plausible in a way that reads correctly on a site with five channels.
...
The "(other)" and "(not set)" support links the donut's tooltips carry are retired with the tooltips. Those two Analytics articles explain why a *slice* is labelled that way; a `(not set)` row in a ranked list beside its share of the total needs no article to be legible, and adding a help icon per row would be noise in a section whose point is that it is scannable.
...
| Channels | `totalUsers` | `sessionDefaultChannelGrouping` | No | `totalUsers` descending |
| Locations | `totalUsers` | `country` | No | `totalUsers` descending |
| Devices | `totalUsers` | `deviceCategory` | No | `totalUsers` descending |
...
`getBreakdownRows( report )` returns `Array<{ label, percentage }>` and applies, in order:

1. Order `report.rows` by visitors descending.  
2. Take the top rows up to the cap, and sum every remaining row's visitors into one trailing "Others" row. Where nothing remains, no "Others" row is added.  
3. Divide each row's visitors — including "Others" — by the sum across every row in the report, and format the result as the row's percentage.

The cap, the "Others" label and the percentage-of-total figure match what `extractAnalyticsDataForPieChart` applies to the donut today, so the same values appear at the top of a dimension before and after the migration.
