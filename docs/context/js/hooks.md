...
function ModuleConnectCTA( { moduleSlug } ) {
	...
	return <button onClick={ activateModule }>Connect { moduleSlug }</button>;
}
```
...
function ExpandableWidget( { widgetSlug } ) {
	...
	useWidgetStateEffect( widgetSlug, isExpanded ? ExpandedComponent : null, {
		expanded: isExpanded,
	} );
...
export default function useModuleSettings( moduleSlug ) {
	...
	// Actions
	const { setSettings, saveSettings, rollbackSettings } = useDispatch(
		`modules/${ moduleSlug }`
	);
...
function ComponentWithPreviousValue( { currentValue } ) {
	...
	useEffect( () => {
		if ( previousValue !== undefined && previousValue !== currentValue ) {
			console.log(
				`Value changed from ${ previousValue } to ${ currentValue }`
			);
		}
