...
For all new code (classes, methods, properties, constants), use `\@since 1.187.0` as a placeholder.
...
The `1.187.0` placeholder will be automatically replaced with the actual version number when a new version is released. This ensures consistent version tracking without needing to know the exact release version during development.
